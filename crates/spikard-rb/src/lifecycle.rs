...
impl LifecycleHook for RubyLifecycleHook {
    ...
    fn execute_request<'a>(
        ...
        Box::pin(async move {
            // Run Ruby code in a blocking task to avoid blocking the async runtime
            let result = tokio::task::spawn_blocking(move || {
                // Serialize request to hash for Ruby
                let (parts, body) = req.into_parts();

                // Read body using tokio runtime in blocking context
                ...
                magnus::Ruby::get()
                    ...
                    .and_then(|ruby| {
                        ...
                        // Call the Ruby function (extract Value from Opaque using get_inner)
                        ...
                        // Check if result is a response (has status_code) or request
                        if let Some(result_hash) = RHash::from_value(result) {
                            // Check for status_code key (response)
                            ...
                            // Add headers if present (skip for now - foreach API complexity)
                            // TODO: Add header iteration support
            ...
            .await
            .map_err(|e| format!("Hook '{}' task error: {}", name_for_error, e))??;
    ...
    fn execute_response<'a>(
        ...
        Box::pin(async move {
            ...
            let result = tokio::task::spawn_blocking(move || {
                // Serialize response to hash for Ruby
