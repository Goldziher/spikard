...
def convert_value(  # noqa: C901, PLR0911, PLR0912
    ...
    *,
    ...
    """Hydrate a single value into ``target_type`` using the same rules as :func:`convert_params`.

    This is used by the ergonomic handler adapter to hydrate a request body (or a
    single typed query/path/header/cookie value) into its declared Python type. It
    mirrors the per-parameter branch of :func:`convert_params`, but operates on one
    value at a time with an already-resolved target type (so ``Body[T]``/``Query[T]``
    wrappers must be unwrapped to ``T`` by the caller).

    Args:
        value: The already-parsed value (dict / list / scalar) to convert.
        target_type: The resolved Python type to hydrate into.
        strict: If True, raise on conversion failures; otherwise fall back to ``value``.
        raw_body: Optional raw JSON bytes; when provided and ``target_type`` is a
            ``msgspec.Struct`` a cached decoder is used for a direct, fast decode.

    Returns:
        The hydrated value, or ``value`` unchanged when no conversion applies.
    """
    if target_type in (None, Any, dict):
        return value
    ...
    file_fields = _get_upload_file_fields(target_type)
    if file_fields and isinstance(value, dict):
        value = _process_upload_file_fields(value, file_fields)
    ...
    if isinstance(target_type, type) and issubclass(target_type, msgspec.Struct):
        try:
            if raw_body:
                decoder = _get_or_create_decoder(target_type)
                return decoder.decode(raw_body)
        except (TypeError, ValueError, msgspec.DecodeError, msgspec.ValidationError):
            if strict:
                raise
        try:
