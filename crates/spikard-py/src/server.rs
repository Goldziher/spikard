...
use crate::handler::{PythonHandler, init_python_event_loop};
...
#[pyfunction]
pub fn create_test_client(py: Python<'_>, app: &Bound<'_, PyAny>) -> PyResult<crate::testing::client::TestClient> {
    ...
    let client = crate::testing::client::TestClient::from_router(axum_router)?;
...
pub fn run_server(py: Python<'_>, app: &Bound<'_, PyAny>, config: &Bound<'_, PyAny>) -> PyResult<()> {
...
#[pyfunction]
#[pyo3(signature = (app, config))]
pub fn run_server_async(py: Python<'_>, app: &Bound<'_, PyAny>, config: &Bound<'_, PyAny>) -> PyResult<Py<PyAny>> {
    let mut config = extract_server_config(py, config)?;
    init_python_event_loop()?;
    let app_router = build_python_server_router(py, app, &mut config)?;
