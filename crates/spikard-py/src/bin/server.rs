//! Spikard Python Server Runner
//!
//! Runs a Python Spikard application with the Rust HTTP server
...
/// Spikard Python Server - Run Python applications with Rust performance
#[derive(Parser, Debug)]
...
fn main() -> Result<()> {
    ...
    run_server(cli.module, cli.host, cli.port)
}
...
fn run_server(module_path: PathBuf, host: String, port: u16) -> Result<()> {
    ...
    let routes_with_handlers = Python::with_gil(|py| -> PyResult<Vec<_spikard::RouteWithHandler>> {
