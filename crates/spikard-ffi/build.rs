...
// alef:hash:f4060e4bb5eac399c5971a2f0ecba70d1916e19c4092ceb9e7fd16b2490d6bbb
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        println!("cargo:rustc-link-arg-cdylib=-Wl,-install_name,@rpath/libspikard_ffi.dylib");
...
fn export_generated_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut header = String::from_utf8(bytes).map_err(|error| {
        io::Error::new(
            ...
            format!("cbindgen generated invalid UTF-8 for include/spikard.h: {error}"),
        )
    })?;

    stamp_enabled_features(&mut header);
    let destinations = [
        Path::new("include/spikard.h"),
        Path::new("../../packages/go/include/spikard.h"),
    ];
...
fn stamp_enabled_features(header: &mut String) {
    ...
    if header.contains(marker) || !header.contains("SPIKARD_FEATURE_") {
        return;
    }
    let mut defines: Vec<String> = std::env::vars_os()
        .filter_map(|(key, _)| {
            ...
            Some(format!("#define SPIKARD_FEATURE_{name} 1"))
        })
        .collect();
    defines.sort();
    defines.dedup();
    let block = format!("\n{marker}\n{}\n", defines.join("\n"));
    let anchor = "#define SPIKARD_H\n";
