            ...
            SchemaKind::Type(Type::Object(obj)) => {
                if obj.properties.is_empty() {
                    ...
                    for (prop_name, prop_schema_ref) in &obj.properties {
                        ...
                        if field_name != prop_name.as_str() {
                            output.push_str(&format!("    #[serde(rename = \"{prop_name}\")]\n"));
                        }
                        ...
                        output.push_str(&format!("    pub {field_name}: {type_hint},\n"));
                    }
                }
            }
            _ => {
...
",
    ...
    fn append_handler(
        ...
        if let Some(req_ty) = request_type {
            handlers.push_str(&format!("    // let body: {req_ty} = ctx.json()?;\n"));
        }
        handlers.push_str(&format!(
            "    println!(\"TODO: Implement {method} {}\");\n",
