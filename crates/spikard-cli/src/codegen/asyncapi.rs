...
pub fn generate_rust_handler_app(spec: &AsyncApiV3Spec, protocol: Protocol) -> Result<String> {
    ...
    code.push_str("use spikard::{App, AppError, RequestContext, get};\n\n");
    ...
    for channel in &channels {
        ...
        let escaped_path = escape_rust_string(&channel.path);
        code.push_str(&format!(
            "    app.route(get(\"{}\").handler_name(\"{}\"), |_ctx: RequestContext| async move {{\n",
            escaped_path, handler_name
        ));
        match protocol {
            ...
            Protocol::Sse => code.push_str(&format!(
                "        // TODO: Stream events for {} on {}\n",
                message_description, channel.path
            )),
            _ => {}
        }
        code.push_str(&format!("        todo!(\"Implement handler for {}\");\n", channel.path));
        code.push_str("    })?;\n\n");
    }
