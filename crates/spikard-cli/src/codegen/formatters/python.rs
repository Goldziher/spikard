...
impl Formatter for PythonFormatter {
    ...
    fn format_imports(&self, imports: &[Import]) -> String {
        ...
        let stdlib_modules = [
            "abc", "argparse", "array", "asyncio", "bisect", "builtins", "calendar", "cmath",
            "cmd", "code", "codeop", "collections", "colorsys", "compileall", "concurrent",
        ...
        if !stdlib_imports.is_empty() {
            for (_, imports_vec) in &stdlib_imports {
        ...
        if !third_party_imports.is_empty() {
            for (_, imports_vec) in &third_party_imports {
                for import in imports_vec {
                    output.push_str(&format_python_import(import));
                    output.push('\n');
                }
        ...
        if !local_imports.is_empty() {
            for (_, imports_vec) in &local_imports {
                for import in imports_vec {
                    output.push_str(&format_python_import(import));
                    output.push('\n');
                }
