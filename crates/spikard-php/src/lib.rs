...
impl App {
    ...
    pub fn on_request(&self, hook: String) -> App {
        Self {
            inner: Arc::new(std::sync::Mutex::new(
                self.inner.lock().unwrap().on_request(hook).clone(),
            )),
        }
    }
    ...
    pub fn pre_validation(&self, hook: String) -> App {
        Self {
            inner: Arc::new(std::sync::Mutex::new(
                self.inner.lock().unwrap().pre_validation(hook).clone(),
            )),
        }
    }
    ...
    pub fn pre_handler(&self, hook: String) -> App {
        Self {
            inner: Arc::new(std::sync::Mutex::new(
                self.inner.lock().unwrap().pre_handler(hook).clone(),
            )),
        }
    }
    ...
    pub fn on_response(&self, hook: String) -> App {
        Self {
            inner: Arc::new(std::sync::Mutex::new(
                self.inner.lock().unwrap().on_response(hook).clone(),
            )),
        }
    }
    ...
    pub fn on_error(&self, hook: String) -> App {
        Self {
            inner: Arc::new(std::sync::Mutex::new(self.inner.lock().unwrap().on_error(hook).clone())),
        }
    }
...
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    ...
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("spikard_php", "0.16.0-rc.4")
