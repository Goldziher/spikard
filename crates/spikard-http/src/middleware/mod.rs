...
/// Extract a multipart boundary token from the first `--<boundary>` line of a
/// body, returning `None` when the bytes do not begin with that pattern.
///
/// Used as a fallback when a client sends `Content-Type: application/json` (or
/// omits the header entirely) with a multipart payload — a pattern generated by
/// the Alef e2e test harness for file-upload fixtures.
        ...
        let out_bytes: bytes::Bytes = match content_kind {
            ...
            Some(validation::ContentTypeKind::Json) | Some(validation::ContentTypeKind::Other) => {
                ...
                if let Some(boundary) = sniff_multipart_boundary(&body_bytes) {
                    ...
                    parts.headers.insert(
                        axum::http::header::CONTENT_TYPE,
                        axum::http::HeaderValue::from_str(&synth_ct)
                            .unwrap_or_else(|_| axum::http::HeaderValue::from_static("multipart/form-data")),
                    );

                    let mut parse_request = HttpRequest::new(Body::from(body_bytes));
                    *parse_request.headers_mut() = parts.headers.clone();

                    let multipart_result = match Multipart::from_request(parse_request, &()).await {
                        Ok(mp) => multipart::parse_multipart_to_json(mp).await,
                        Err(e) => {
                            let error_body = json!({
                                "error": format!("Failed to parse multipart data: {}", e)
                            });
                    ...
                    let json_body = match multipart_result {
                        Ok(json) => json,
                        Err(e) => {
                            let error_body = json!({
                                "error": format!("Failed to process multipart data: {}", e)
