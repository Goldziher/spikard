...
type JsHandlerFn = napi::threadsafe_function::ThreadsafeFunction<serde_json::Value, serde_json::Value>;

/// Spikard application builder.
/// Stores route registrations (RouteBuilder + handler) and ServerConfig,
/// then builds and runs the Axum server on `nativeRun()`.
#[napi(js_name = "App")]
pub struct JsApp {
    routes: Arc<std::sync::Mutex<Vec<(spikard::RouteBuilder, JsHandlerFn)>>>,
    server_config: Arc<std::sync::Mutex<spikard::ServerConfig>>,
}
...
impl JsApp {
    ...
    pub fn new() -> JsApp {
        Self {
            ...
            server_config: Arc::new(std::sync::Mutex::new(spikard::ServerConfig::default())),
        }
    }
    ...
    /// Set the server configuration (host, port, compression, etc.).
    #[napi(js_name = "setConfig")]
    pub fn set_config(&self, cfg: JsServerConfig) -> napi::Result<()> {
        let core_cfg: spikard::ServerConfig = cfg.into();
        let mut sc = self.server_config.lock().expect("server_config mutex poisoned");
        *sc = core_cfg;
        Ok(())
    }

    /// Register a route using a RouteBuilder and a handler function.
    #[napi(js_name = "nativeRegisterRoute")]
    pub fn native_register_route(
        &self,
        builder: &JsRouteBuilder,
        handler: JsHandlerFn,
    ) -> napi::Result<()> {
        let route_builder: spikard::RouteBuilder = (*builder.inner).clone();
        let mut routes = self.routes.lock().expect("routes mutex poisoned");
        routes.push((route_builder, handler));
        Ok(())
    }

    /// Run the HTTP server with configured routes and ServerConfig.
    #[napi(js_name = "nativeRun")]
    pub async fn native_run(&self) -> napi::Result<()> {
        use crate::service::HandlerBridge;

        // Take all registrations out of the mutex (replaces with empty Vec).
        // The server can only be started once; subsequent calls would start with no routes.
        let registrations = {
            let mut routes = self.routes.lock().expect("routes mutex poisoned");
            std::mem::take(&mut *routes)
        };
        let cfg = {
            let sc = self.server_config.lock().expect("server_config mutex poisoned");
            (*sc).clone()
        };

        let mut app = spikard::App::new();
        for (route_builder, handler_fn) in registrations {
            let bridge = HandlerBridge::new(handler_fn);
            let handler_arc: Arc<dyn spikard::Handler> = Arc::new(bridge);
            app.route(route_builder, handler_arc)
