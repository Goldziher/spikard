...
- **Ruby/PHP gRPC helper status normalization**: Ruby and PHP gRPC response helpers now normalize named status aliases like `UNAUTHENTICATED` to numeric `grpc-status` metadata on emitted responses, keeping helper ergonomics without violating gRPC wire semantics.
