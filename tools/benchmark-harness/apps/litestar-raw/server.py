"""Litestar HTTP server for workload benchmarking (raw/no validation).

This server implements all workload types to measure Litestar's RAW performance
without msgspec validation overhead. All request parsing is manual.

This is a baseline to measure Litestar framework overhead vs. litestar/server.py
which uses msgspec.Struct validation.
"""
# /// script
# dependencies = [
#     "litestar",
...
async def post_json_medium(request: Request) -> dict[str, Any]:
    """Medium JSON payload (~1-10KB) - manual parsing."""
    data = await request.json()
    return data
...
async def post_json_large(request: Request) -> dict[str, Any]:
    """Large JSON payload (~10-100KB) - manual parsing."""
    data = await request.json()
    return data
...
async def post_json_very_large(request: Request) -> dict[str, Any]:
    """Very large JSON payload (~100KB-1MB) - manual parsing."""
    data = await request.json()
    return data
...
# ============================================================================
# URL Encoded Form Workloads - Manual Parsing
# ============================================================================


@post("/urlencoded/simple", sync_to_thread=False)
async def post_urlencoded_simple(request: Request) -> dict[str, str]:
    """Simple URL-encoded form (3-5 fields) - manual parsing."""
...
async def post_urlencoded_complex(request: Request) -> dict[str, Any]:
    """Complex URL-encoded form (10-20 fields) - manual parsing."""
    data = await request.form()
    return {
        "username": data.get("username", ""),
        "email": data.get("email", ""),
        ...
        "newsletter": data.get("newsletter", "false") == "true",
        "notifications": data.get("notifications", "false") == "true",
...
if __name__ == "__main__":
    ...
    print(
        f"Litestar workload server (raw/no validation) starting on port {port}",
        file=sys.stderr,
        flush=True,
    )
    ...
    uvicorn.run(
        app,
        host="0.0.0.0",  # MUST be 0.0.0.0 for fair comparison, not 127.0.0.1
        port=port,
