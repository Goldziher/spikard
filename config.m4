dnl Configuration for Rust-based PHP extension via ext-php-rs.
dnl Allows phpize to recognize this extension during source compilation (PIE fallback).

PHP_ARG_ENABLE([spikard_php],
  [whether to enable the spikard_php extension],
  [AS_HELP_STRING([--enable-spikard_php],
    [Enable spikard_php extension support])],
  [yes])

if test "$PHP_SPIKARD_PHP_ENABLED" = "yes"; then
  dnl Register the extension directory so phpize creates modules/ and sets up build rules.
  PHP_NEW_EXTENSION(spikard_php, [], $ext_shared)

  dnl Invoke cargo build to compile the Rust FFI library and copy it to modules/.
  AC_CONFIG_COMMANDS([cargo-build], [
    if test -f "crates/spikard-php/Cargo.toml"; then
      (cd crates/spikard-php && cargo build --release) || exit 1

      dnl Detect output filename based on platform
      if test -f "crates/spikard-php/target/release/libspikard-php_php.dylib"; then
        cargo_lib="crates/spikard-php/target/release/libspikard-php_php.dylib"
      elif test -f "crates/spikard-php/target/release/libspikard-php_php.so"; then
        cargo_lib="crates/spikard-php/target/release/libspikard_php.so"
      else
        echo "ERROR: cargo build succeeded but .so/.dylib not found in crates/spikard_php-php/target/release" >&2
        exit 1
      fi

      mkdir -p modules
      cp "$cargo_lib" "modules/spikard.so" || exit 1
    else
      echo "ERROR: crates/spikard_php-php/Cargo.toml not found" >&2
      exit 1
    fi
  ], [])
fi
