 ...
 */
get("/config")(async function getConfig(
  request,
  { app_name, version }: { app_name: string; version: string },
) {
  return {
    app: app_name,
    version: version,
...
get("/db-info")(async function getDbInfo(
  request,
  {
    app_name,
    db_pool,
  }: { app_name: string; db_pool: { connected: boolean; url: string; maxConnections: number } },
) {
  return {
    app: app_name,
    database: {
