...
async function runClient() {
  ...
  const streamData = JSON.parse(
    streamResponse.payload.toString("utf-8"),
  ) as StockPriceStreamResponse;
...
async function runServer() {
  ...
  // grpcService.registerUnary('stock.v1.StockService', 'GetPrice', new StockPriceServiceHandler());
  // grpcService.registerServerStreaming('stock.v1.StockService', 'StreamPrices', new StockPriceServiceHandler());
...
export {
  StockPriceServiceHandler,
  type StockPriceRequest,
  type StockPrice,
  type StockPriceStreamResponse,
};
