...
async function runClient() {
  ...
  streamData.prices.forEach((price, i) => {
    console.log(`  [${i + 1}] ${price.symbol}: $${price.price.toFixed(2)} @ ${new Date(price.timestamp).toISOString()}`);
  });
...
export { StockPriceServiceHandler, StockPriceRequest, StockPrice, StockPriceStreamResponse };
