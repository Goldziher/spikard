...
class DataServiceHandler {
	 ...
	 */
	async handleClientStream(requestStream: AsyncIterator<GrpcRequest>): Promise<GrpcResponse> {
		...
		try {
			for await (const request of requestStream) {
				...
				if (requestCount > MAX_BATCH_SIZE) {
					throw new Error(`RESOURCE_EXHAUSTED: Batch size exceeded (max ${MAX_BATCH_SIZE})`);
				}
	...
	private validateInput(payload: DataRequest): void {
		...
		if (payload.query.length > this.MAX_QUERY_LENGTH) {
			throw new Error(`INVALID_ARGUMENT: query exceeds maximum length of ${this.MAX_QUERY_LENGTH}`);
		}
...
async function runClient() {
	...
	try {
		await service.handleRequest({
			serviceName: "data.v1.DataService",
			methodName: "GetData",
			payload: Buffer.from(JSON.stringify({ userId: "guest-user1", query: "sensitive data" }), "utf-8"),
			metadata: { authorization: "Bearer valid-token-12345" },
		});
		console.log("✗ Should have failed\n");
	} catch (error) {
	...
	try {
		await service.handleServerStream({
			...
			payload: Buffer.from(JSON.stringify({ userId: "user2", query: "stream test", limit: 10 }), "utf-8"),
			metadata: { authorization: "Bearer valid-token-12345" },
		});
	...
	try {
		const response = await service.handleRequest({
			serviceName: "data.v1.DataService",
			methodName: "GetData",
			payload: Buffer.from(JSON.stringify({ userId: "user3", query: "valid query" }), "utf-8"),
			metadata: { authorization: "Bearer valid-token-12345" },
		});
