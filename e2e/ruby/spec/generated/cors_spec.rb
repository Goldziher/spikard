...
RSpec.describe "cors" do
  it "06_cors_preflight_method_not_allowed" do
    ...
    response = client.options(path, headers: {"Access-Control-Request-Headers" => "Content-Type", "Access-Control-Request-Method" => "DELETE", "Origin" => "https://example.com"})
  ...
  it "07_cors_preflight_header_not_allowed" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.options(path, headers: {"Access-Control-Request-Headers" => "X-Custom-Header", "Access-Control-Request-Method" => "POST", "Origin" => "https://example.com"})
    expect(response.status_code).to eq(403)
    expect(response.body_text).to be_nil
    client.close
  end
  ...
  it "CORS preflight request" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.options(path, headers: {"Access-Control-Request-Headers" => "Content-Type, X-Custom-Header", "Access-Control-Request-Method" => "POST", "Origin" => "https://example.com"})
    expect(response.status_code).to eq(200)
    expect(response.body_text).to be_nil
    client.close
  end
  ...
  it "CORS request blocked" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.get(path, headers: {"Origin" => "https://malicious-site.com"})
    expect(response.status_code).to eq(403)
  ...
  it "CORS wildcard origin" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.get(path, headers: {"Origin" => "https://random-site.com"})
    expect(response.status_code).to eq(200)
  ...
  it "CORS with credentials" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.get(path, headers: {"Cookie" => "session=abc123", "Origin" => "https://app.example.com"})
    expect(response.status_code).to eq(200)
