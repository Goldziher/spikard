...
RSpec.describe "query_params" do
  ...
  it "Integer with default value - not provided" do
    ...
    expect(response.status_code).to eq(200)
    expect(response.body_text).to eq("foo bar 10")
    client.close
  end
  ...
  it "Integer with default value - override" do
    ...
    expect(response.status_code).to eq(200)
    expect(response.body_text).to eq("foo bar 50")
    client.close
  end
  ...
  it "Optional integer query parameter - missing" do
    ...
    expect(response.status_code).to eq(200)
    expect(response.body_text).to eq("foo bar None")
    client.close
  end
  ...
  it "Optional string query parameter - provided" do
    ...
    expect(response.status_code).to eq(200)
    expect(response.body_text).to eq("foo bar baz")
    client.close
  end
  ...
  it "Required integer query parameter - success" do
    ...
    expect(response.status_code).to eq(200)
    expect(response.body_text).to eq("foo bar 42")
    client.close
  end
