...
RSpec.describe "content_types" do
  it "13_json_with_charset_utf16" do
    ...
    response = client.post(path, headers: {"Content-Type" => "application/json; charset=utf-16"}, json: {"value" => "test"})
  ...
  it "14_content_type_case_insensitive" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "APPLICATION/JSON"}, raw_body: "{\"name\":\"test\"}")
  ...
  it "15_multipart_boundary_required" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "multipart/form-data"})
  ...
  it "16_text_plain_not_accepted" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "text/plain"}, raw_body: "{\"data\": \"value\"}")
  ...
  it "17_vendor_json_accepted" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "application/vnd.api+json"}, json: {"data" => "value"})
    expect(response.status_code).to eq(201)
  ...
  it "18_content_type_with_multiple_params" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "application/json; charset=utf-8; boundary=something"}, json: {"value" => "test"})
    expect(response.status_code).to eq(201)
  ...
  it "20_content_length_mismatch" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Length" => "100", "Content-Type" => "application/json"}, json: {"value" => "short"})
    expect(response.status_code).to eq(400)
  ...
  it "415 Unsupported Media Type" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, headers: {"Content-Type" => "application/xml"}, raw_body: "<?xml version=\"1.0\"?><item><name>Item</name></item>")
    expect(response.status_code).to eq(415)
