...
RSpec.describe "multipart" do
  ...
  it "Form data without files" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, data: {"some" => "data"})
    expect(response.status_code).to eq(200)
  ...
  it "Mixed files and form data" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, data: {"active" => "true", "age" => "25", "username" => "testuser"})
    expect(response.status_code).to eq(200)
  ...
  it "Multiple values for same field name" do
    ...
    client = Spikard::Testing.create_test_client(app)
    path = app.route_metadata.first[:path]
    response = client.post(path, data: {"tags" => ["python", "rust", "web"]})
    expect(response.status_code).to eq(200)
