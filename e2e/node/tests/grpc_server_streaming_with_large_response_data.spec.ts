...
import { handleGrpcServerStreamingWithLargeResponseData } from "../app/main.ts";

describe("grpc", () => {
	test("should handle gRPC request: Server streaming with large response data", async () => {
		...
		expect(response.payload).toEqual(
			Buffer.from(JSON.stringify({ stream_id: "stream-large-001", chunk_number: 1, is_final: false })),
		);
