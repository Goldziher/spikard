...
// alef:hash:2a57118bfb518800d3290f95bd7cb204fd77358187f37d164f23e4b433dc765c
...
describe('content_types', () => {
  ...
  it('_missing_content_type_default_json: Missing Content-Type header should default to JSON when body is present', async () => {
    ...
    const response = await fetch(mockUrl, { method: 'POST', body: JSON.stringify({ name: "test" }) });
    expect(response.status).toBe(201);
    const data = await response.json();
    expect(data).toEqual({ name: "test" });
  });
  ...
  it('_content_length_mismatch: Content-Length header mismatch with actual body size should fail', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/20_content_length_mismatch`;
    const response = await fetch(mockUrl, { method: 'POST', headers: {
      "Content-Type": "application/json",
      "Content-Length": "100",
