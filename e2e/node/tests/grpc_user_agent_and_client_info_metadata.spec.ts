...
import { handleGrpcUserAgentAndClientInfoMetadata } from "../app/main.ts";

describe("grpc", () => {
	test("should handle gRPC request: User-Agent and client info metadata", async () => {
		...
		expect(response.payload).toEqual(
			Buffer.from(JSON.stringify({ client_type: "grpc-client", client_version: "1.2.3" })),
		);
