...
// alef:hash:8212a1be890cee62c5b06a6cb1874bf6312a05afe48fe968c4830e71fce290c8
...
describe("compression", () => {
	it("compression_brotli_only: Tests Brotli compression when enabled and client supports it", async () => {
		const response = await app.request("/compression/brotli", {
			method: "GET",
			headers: {
				"Accept-Encoding": "br",
			},
	...
	it("compression_min_size_threshold_exact_boundary: Tests compression respects exact minimum size boundary", async () => {
		const response = await app.request("/compression/boundary", {
			method: "GET",
			headers: {
				"Accept-Encoding": "gzip",
			},
	...
	it("compression_quality_level_9: Tests compression with high quality level setting", async () => {
		const response = await app.request("/compression/high_quality", {
			method: "GET",
			headers: {
				"Accept-Encoding": "gzip",
			},
