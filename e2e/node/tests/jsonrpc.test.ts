...
// alef:hash:b91986c1257e283f2796f14bcc15bd3ddd5127430261a615668100b1cd59551c
...
describe('jsonrpc', () => {  it('jsonrpc_batch_request: Tests JSON-RPC batch request with multiple calls', async () => {
  ...
  it('jsonrpc_deprecated_method_flag: Tests JSON-RPC method marked as deprecated', async () => {
    ...
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ id: 1, jsonrpc: "2.0", result: "deprecated" });    expect(response.headers.get('deprecation')).toBe('true');    expect(response.headers.get('sunset')).not.toBeNull();  });
