//! Tests for body_limits fixtures
//! Generated from: testing_data/body_limits
...
mod body_limits {
    ...
    async fn test_body_limits_body_over_limit_returns_413() {
        // Fixture: Body over limit returns 413
        // Description: Requests that exceed the configured max body size should be rejected with Payload Too Large.
        // Expected status: 413

        // Load fixture
        let fixture_json = std::fs::read_to_string("../../testing_data/body_limits/02_over_limit.json")
    ...
    #[tokio::test]
    async fn test_body_limits_body_under_limit_succeeds() {
        // Fixture: Body under limit succeeds
        // Description: Ensures requests smaller than the configured body limit are accepted.
        // Expected status: 200

        // Load fixture
        let fixture_json = std::fs::read_to_string("../../testing_data/body_limits/01_within_limit.json")
            .expect("Failed to read fixture file");
        let fixture: Value = serde_json::from_str(&fixture_json).expect("Failed to parse fixture JSON");
