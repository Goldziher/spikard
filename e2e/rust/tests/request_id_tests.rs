//! Tests for request_id fixtures
//! Generated from: testing_data/request_id
...
#[tokio::test]
async fn test_request_id_request_id_header_is_preserved() {
    // Fixture: Request ID header is preserved
    // Description: When the client supplies X-Request-ID the same value should appear on the response.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/request_id/02_propagates_existing_header.json")
    ...
    let app = spikard_e2e_app::create_app_request_id_request_id_header_is_preserved()
        .expect("Failed to build fixture app");
    let server = TestServer::from_app(app).expect("Failed to build server");

    // Build request
    let mut uri = "/request-id/preserved".to_string();
    ...
    if let Some(actual) = headers.get("x-request-id") {
        assert_eq!(actual, "trace-123", "Mismatched header 'x-request-id'");
    } else {
...
#[tokio::test]
async fn test_request_id_request_id_is_generated_when_not_provided() {
    // Fixture: Request ID is generated when not provided
    // Description: Ensures the request ID middleware attaches a UUID to responses when the client does not send one.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/request_id/01_generates_request_id.json")
        .expect("Failed to read fixture file");
    let fixture: Value = serde_json::from_str(&fixture_json)
        .expect("Failed to parse fixture JSON");

    // Create app for this specific fixture
    let app = spikard_e2e_app::create_app_request_id_request_id_is_generated_when_not_provided()
        .expect("Failed to build fixture app");
    let server = TestServer::from_app(app).expect("Failed to build server");

    // Build request
    let mut uri = "/request-id/generated".to_string();
    ...
    if let Some(value) = headers.get("x-request-id") {
        assert!(uuid::Uuid::parse_str(value).is_ok(), "Expected header 'x-request-id' to be a UUID, got {}", value);
    } else {
        panic!("Expected header 'x-request-id' to be present");
    }
...
#[tokio::test]
async fn test_request_id_request_id_middleware_can_be_disabled() {
    // Fixture: Request ID middleware can be disabled
    // Description: When request ID generation is disabled the response should not contain X-Request-ID even if the client sends a header.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/request_id/03_disabled_request_id.json")
        .expect("Failed to read fixture file");
    let fixture: Value = serde_json::from_str(&fixture_json)
        .expect("Failed to parse fixture JSON");

    // Create app for this specific fixture
    let app = spikard_e2e_app::create_app_request_id_request_id_middleware_can_be_disabled()
        .expect("Failed to build fixture app");
    let server = TestServer::from_app(app).expect("Failed to build server");

    // Build request
    let mut uri = "/request-id/disabled".to_string();
    ...
    , "Expected header 'x-request-id' to be absent, got {:?}", headers.get("x-request-id"));
