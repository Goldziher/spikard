//! Tests for compression fixtures
//! Generated from: testing_data/compression
...
async fn test_compression_compression_gzip_applied() {
    // Fixture: Compression - gzip applied
    // Description: Serves a JSON payload compressed with gzip when the client advertises support.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/compression/01_gzip_applied.json")
    ...
    if let Some(actual) = headers.get("vary") {
        assert_eq!(actual.to_ascii_lowercase(), "Accept-Encoding".to_ascii_lowercase(), "Mismatched header 'vary'");
    } else {
...
#[tokio::test]
async fn test_compression_compression_payload_below_min_size_is_not_compressed() {
    // Fixture: Compression - payload below min_size is not compressed
    // Description: Ensures responses smaller than the configured min_size are sent uncompressed even when the client sends Accept-Encoding.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/compression/02_min_size_skip.json")
        .expect("Failed to read fixture file");
    let fixture: Value = serde_json::from_str(&fixture_json)
        .expect("Failed to parse fixture JSON");
    ...
    , "Expected header 'content-encoding' to be absent, got {:?}", headers.get("content-encoding"));
