//! Tests for request_timeout fixtures
//! Generated from: testing_data/request_timeout
...
async fn test_request_timeout_request_completes_before_timeout() {
    // Fixture: Request completes before timeout
    // Description: Simulated handler sleeps briefly and should complete before the timeout middleware fires.
    // Expected status: 200

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/request_timeout/01_completes_before_deadline.json")
...
#[tokio::test]
async fn test_request_timeout_request_exceeds_timeout() {
    // Fixture: Request exceeds timeout
    // Description: Simulates a handler that sleeps longer than the configured timeout to ensure a 408 response.
    // Expected status: 408

    // Load fixture
    let fixture_json = std::fs::read_to_string("../../testing_data/request_timeout/02_times_out.json")
        .expect("Failed to read fixture file");
    let fixture: Value = serde_json::from_str(&fixture_json)
        .expect("Failed to parse fixture JSON");
