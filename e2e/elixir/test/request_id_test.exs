...
defmodule E2e.RequestIdTest do
  ...
  describe "request_id_header_is_preserved" do
    test "When the client supplies X-Request-ID the same value should appear on the response." do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/request_id_header_is_preserved",
          finch: AlefE2EFinch,
  ...
  describe "request_id_is_generated_when_not_provided" do
    test "Ensures the request ID middleware attaches a UUID to responses when the client does not send one." do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/request_id_is_generated_when_not_provided",
          finch: AlefE2EFinch
        )
      ...
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "x-request-id", do: List.first(List.wrap(v)) end) ==
               "a4c20a2c-3b1f-4e2d-8b1d-3c9a1f2e4d5a"
  ...
  describe "request_id_middleware_can_be_disabled" do
    test "When request ID generation is disabled the response should not contain X-Request-ID even if the client sends a header." do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/request_id_middleware_can_be_disabled",
          finch: AlefE2EFinch,
