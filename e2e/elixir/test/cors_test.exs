...
# alef:hash:d629d25212c310eb543352172fbcb0cec38311384ec244612fa67ff90aa7688c
...
defmodule E2e.CorsTest do
  ...
  describe "_cors_preflight_method_not_allowed" do
    test "OPTIONS /api/data - CORS preflight request for non-allowed method should be rejected" do
      {:ok, response} = Req.options(url: "#{mock_server_url()}/fixtures/06_cors_preflight_method_not_allowed", headers: [{"Access-Control-Request-Headers", "Content-Type"}, {"Origin", "https://example.com"}, {"Access-Control-Request-Method", "DELETE"}])
  ...
  describe "_cors_preflight_header_not_allowed" do
    test "OPTIONS /api/data - CORS preflight request with non-allowed header should be rejected" do
      {:ok, response} = Req.options(url: "#{mock_server_url()}/fixtures/07_cors_preflight_header_not_allowed", headers: [{"Access-Control-Request-Method", "POST"}, {"Origin", "https://example.com"}, {"Access-Control-Request-Headers", "X-Custom-Header"}])
      assert response.status == 403
    end
  end
  ...
  describe "_cors_max_age" do
    test "OPTIONS /api/data - CORS preflight response should include Access-Control-Max-Age" do
      {:ok, response} = Req.options(url: "#{mock_server_url()}/fixtures/08_cors_max_age", headers: [{"Access-Control-Request-Method", "POST"}, {"Origin", "https://example.com"}, {"Access-Control-Request-Headers", "Content-Type"}])
  ...
  describe "cors_custom_allowed_headers_x_custom" do
    test "OPTIONS /api/data - Tests CORS allows custom X-Custom-Header in requests" do
      {:ok, response} = Req.options(url: "#{mock_server_url()}/fixtures/cors_custom_allowed_headers_x_custom", headers: [{"Access-Control-Request-Method", "POST"}, {"Origin", "https://example.com"}, {"Access-Control-Request-Headers", "X-Custom-Header"}])
      assert response.status == 204
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "access-control-allow-origin", do: v end) == "https://example.com"
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "access-control-allow-methods", do: v end) == "POST"
  ...
  describe "cors_safelisted_headers_without_preflight" do
    test "POST /api/form - Tests that safelisted headers (Content-Type: text/plain, Accept, Accept-Language) don't require preflight" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/cors_safelisted_headers_without_preflight", headers: [{"Origin", "https://app.example.com"}, {"Accept", "application/json"}, {"Accept-Language", "en-US"}, {"Content-Type", "text/plain"}])
      assert response.status == 200
      assert Jason.decode!(response.body) == %{"message" => "Success"}
