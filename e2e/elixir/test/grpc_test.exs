...
# alef:hash:d55423dec2437d726134e02e967beb0d47b34d1ede719572d30fc9bda5be6829
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"x-request-id", "req-12345"}, {"te", "trailers"}, {"Content-Type", "application/grpc"}, {"x-trace-id", "trace-abc123"}, {"x-user-id", "user-42"}])
      assert response.status == 200
