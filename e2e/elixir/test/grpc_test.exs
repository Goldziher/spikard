...
# alef:hash:60a76f73c6c1bd5e2078f9e42f33b9b81526ce33808a894b1373e85de830d376
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_call_with_metadata", json: %{"id" => "resource-1"}, headers: [{"x-custom-metadata", "value123"}, {"authorization", "Bearer token123"}, {"Content-Type", "application/grpc"}, {"te", "trailers"}])
  ...
  describe "grpc_deadline_exceeded" do
    test "POST /api.Service/SlowOperation - Tests gRPC DEADLINE_EXCEEDED error when timeout occurs" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_deadline_exceeded", json: %{"duration_ms" => 60000}, headers: [{"te", "trailers"}, {"grpc-timeout", "100m"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_request_with_custom_metadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}, {"x-user-id", "user-42"}, {"x-request-id", "req-12345"}, {"x-trace-id", "trace-abc123"}])
      assert response.status == 200
  ...
  describe "grpc_unary_call_success" do
    test "POST /helloworld.Greeter/SayHello - Tests unary gRPC call with successful response" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_unary_call_success", json: %{"name" => "World"}, headers: [{"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
