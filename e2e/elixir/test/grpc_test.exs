...
# alef:hash:07139c5ee3a74ca44c00694acedd9e1c7db2ad40cf04942639aca94326cee718
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetData", json: %{"id" => "resource-1"}, headers: [{"x-custom-metadata", "value123"}, {"authorization", "Bearer token123"}, {"Content-Type", "application/grpc"}, {"te", "trailers"}])
  ...
  describe "grpc_config_request_timeout" do
    test "POST /api.Service/TimedOperation - Tests gRPC request timeout configuration" do
      {:ok, response} = Req.post(client(), url: "/api.Service/TimedOperation", json: %{"operation" => "process"}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}, {"grpc-timeout", "5000m"}])
      assert response.status == 200
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "grpc-status", do: v end) == "0"
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "content-type", do: v end) == "application/grpc"
    end
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"Content-Type", "application/grpc"}, {"x-user-id", "user-42"}, {"x-request-id", "req-12345"}, {"te", "trailers"}, {"x-trace-id", "trace-abc123"}])
      assert response.status == 200
