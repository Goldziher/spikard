...
# alef:hash:437166ad051e355ae6561ccc336897271beb995af1eae56d06e9e1d35e20cd84
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetData", json: %{"id" => "resource-1"}, headers: [{"x-custom-metadata", "value123"}, {"te", "trailers"}, {"authorization", "Bearer token123"}, {"Content-Type", "application/grpc"}])
  ...
  describe "grpc_config_compression_enabled" do
    test "POST /api.Service/GetCompressedData - Tests gRPC compression is enabled for responses" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetCompressedData", json: %{"request" => "get_data"}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}, {"grpc-encoding", "gzip"}])
      assert response.status == 200
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"Content-Type", "application/grpc"}, {"x-trace-id", "trace-abc123"}, {"te", "trailers"}, {"x-user-id", "user-42"}, {"x-request-id", "req-12345"}])
      assert response.status == 200
