...
# alef:hash:60dcdd88f680f583ec08d490b7830cb7318360b9c8bf9294380645bd66e2656c
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetData", json: %{"id" => "resource-1"}, headers: [{"authorization", "Bearer token123"}, {"x-custom-metadata", "value123"}, {"te", "trailers"}, {"Content-Type", "application/grpc"}])
  ...
  describe "grpc_config_compression_enabled" do
    test "POST /api.Service/GetCompressedData - Tests gRPC compression is enabled for responses" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetCompressedData", json: %{"request" => "get_data"}, headers: [{"te", "trailers"}, {"grpc-encoding", "gzip"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "content-type", do: v end) == "application/grpc"
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "grpc-status", do: v end) == "0"
  ...
  describe "grpc_config_max_message_size" do
    test "POST /api.Service/SendLargeMessage - Tests gRPC server respects maximum message size configuration" do
      {:ok, response} = Req.post(client(), url: "/api.Service/SendLargeMessage", json: %{"data" => "large_payload_data"}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}])
      assert response.status == 200
  ...
  describe "grpc_config_request_timeout" do
    test "POST /api.Service/TimedOperation - Tests gRPC request timeout configuration" do
      {:ok, response} = Req.post(client(), url: "/api.Service/TimedOperation", json: %{"operation" => "process"}, headers: [{"te", "trailers"}, {"grpc-timeout", "5000m"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "content-type", do: v end) == "application/grpc"
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "grpc-status", do: v end) == "0"
    end
  ...
  describe "grpc_deadline_exceeded" do
    test "POST /api.Service/SlowOperation - Tests gRPC DEADLINE_EXCEEDED error when timeout occurs" do
      {:ok, response} = Req.post(client(), url: "/api.Service/SlowOperation", json: %{"duration_ms" => 60000}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}, {"grpc-timeout", "100m"}])
      assert response.status == 200
  ...
  describe "grpc_error_not_found" do
    test "POST /api.Service/GetItem - Tests gRPC NOT_FOUND error status response" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetItem", json: %{"item_id" => "nonexistent"}, headers: [{"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"Content-Type", "application/grpc"}, {"te", "trailers"}, {"x-user-id", "user-42"}, {"x-trace-id", "trace-abc123"}, {"x-request-id", "req-12345"}])
      assert response.status == 200
  ...
  describe "grpc_response_with_metadata_trailers" do
    test "POST /api.Service/MetadataResponse - Tests gRPC response includes metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/MetadataResponse", json: %{"request" => "test"}, headers: [{"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
