...
# alef:hash:8fe864de713029b4dcac051970aa6fb247fccd1bab7d4b32b38a0e0cb947b72e
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"x-request-id", "req-12345"}, {"x-user-id", "user-42"}, {"te", "trailers"}, {"x-trace-id", "trace-abc123"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
