...
# alef:hash:cee254f6b6848c08c176e7a7af51cfa412f9780083b4cefcfc87372e634667ad
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/GetData", json: %{"id" => "resource-1"}, headers: [{"Content-Type", "application/grpc"}, {"x-custom-metadata", "value123"}, {"authorization", "Bearer token123"}, {"te", "trailers"}])
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(client(), url: "/api.Service/ProcessWithMetadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"Content-Type", "application/grpc"}, {"x-trace-id", "trace-abc123"}, {"te", "trailers"}, {"x-request-id", "req-12345"}, {"x-user-id", "user-42"}])
      assert response.status == 200
