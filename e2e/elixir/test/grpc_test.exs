...
# alef:hash:3e4451def03e510fcac643f9519f56907145d540c057d7dce39b76d2c771425f
...
defmodule E2e.GrpcTest do
  ...
  describe "grpc_call_with_metadata" do
    test "POST /api.Service/GetData - Tests gRPC call with request metadata headers" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_call_with_metadata", json: %{"id" => "resource-1"}, headers: [{"te", "trailers"}, {"authorization", "Bearer token123"}, {"x-custom-metadata", "value123"}, {"Content-Type", "application/grpc"}])
  ...
  describe "grpc_config_compression_enabled" do
    test "POST /api.Service/GetCompressedData - Tests gRPC compression is enabled for responses" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_config_compression_enabled", json: %{"request" => "get_data"}, headers: [{"grpc-encoding", "gzip"}, {"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
  ...
  describe "grpc_config_max_message_size" do
    test "POST /api.Service/SendLargeMessage - Tests gRPC server respects maximum message size configuration" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_config_max_message_size", json: %{"data" => "large_payload_data"}, headers: [{"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
  ...
  describe "grpc_error_invalid_argument" do
    test "POST /api.Service/ValidateInput - Tests gRPC INVALID_ARGUMENT error status response" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_error_invalid_argument", json: %{"value" => -100}, headers: [{"te", "trailers"}, {"Content-Type", "application/grpc"}])
      assert response.status == 200
  ...
  describe "grpc_request_with_custom_metadata" do
    test "POST /api.Service/ProcessWithMetadata - Tests gRPC request includes custom metadata in trailers" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/grpc_request_with_custom_metadata", json: %{"data" => %{"key" => "value"}, "request_id" => "req-12345"}, headers: [{"x-trace-id", "trace-abc123"}, {"te", "trailers"}, {"x-user-id", "user-42"}, {"Content-Type", "application/grpc"}, {"x-request-id", "req-12345"}])
      assert response.status == 200
