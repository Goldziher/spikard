...
defmodule E2e.StatusCodesTest do
  ...
  describe "413_payload_too_large" do
    test "Request with body exceeding max size should return 413" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/19_413_payload_too_large",
          finch: AlefE2EFinch,
          json: %{
            "data" =>
  ...
  describe "created_resource_created" do
    test "Tests 201 Created response for successful POST request" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/201_created_resource_created",
          finch: AlefE2EFinch,
          json: %{"name" => "New Item"},
  ...
  describe "accepted_request_accepted_for_processing" do
    test "Tests 202 Accepted for async processing" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/202_accepted_request_accepted_for_processing",
          finch: AlefE2EFinch,
  ...
  describe "no_content_success_with_no_body" do
    test "Tests 204 No Content response for successful DELETE" do
      {:ok, response} =
        Req.delete(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/204_no_content_success_with_no_body",
          finch: AlefE2EFinch
        )
  ...
  describe "414_uri_too_long" do
    test "Request with excessively long URI should return 414" do
      {:ok, response} =
        Req.get(url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/20_414_uri_too_long", finch: AlefE2EFinch)
  ...
  describe "431_request_header_fields_too_large" do
    test "Request with excessively large headers should return 431" do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/21_431_request_header_fields_too_large",
          finch: AlefE2EFinch,
          headers: [
            {"X-Large-Header",
  ...
  describe "503_service_unavailable" do
    test "Service temporarily unavailable should return 503 with Retry-After" do
      {:ok, response} =
        Req.get(url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/23_503_service_unavailable", finch: AlefE2EFinch)
  ...
  describe "not_modified_cached_content_valid" do
    test "Tests 304 Not Modified for cached resources" do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/304_not_modified_cached_content_valid",
          finch: AlefE2EFinch,
  ...
  describe "temporary_redirect_method_preserved" do
    test "Tests 307 temporary redirect with method preservation" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/307_temporary_redirect_method_preserved",
          finch: AlefE2EFinch,
          json: %{},
          headers: [{"Content-Type", "application/json"}],
          redirect: false
      ...
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "location", do: List.first(List.wrap(v)) end) ==
               "/target-post"
  ...
  describe "bad_request_invalid_request" do
    test "Tests 400 Bad Request for malformed request" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/400_bad_request_invalid_request",
          finch: AlefE2EFinch,
          json: "not valid json",
          headers: [{"Content-Type", "application/json"}]
        )
  ...
  describe "not_found_resource_not_found" do
    test "Tests 404 Not Found for non-existent resource" do
      {:ok, response} =
        Req.get(url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/404_not_found_resource_not_found", finch: AlefE2EFinch)
  ...
  describe "request_timeout" do
    test "Tests 408 status code when request takes too long" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/408_request_timeout",
          finch: AlefE2EFinch,
  ...
  describe "unprocessable_entity_validation_error" do
    test "Tests 422 for validation errors (Pydantic)" do
      {:ok, response} =
        Req.post(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/422_unprocessable_entity_validation_error",
          finch: AlefE2EFinch,
