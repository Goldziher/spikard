...
# alef:hash:304e55eadd8a55c5aefec2569931d19d58e3cff6d1946aac4d133b86ec262906
...
defmodule E2e.LifecycleHooksTest do
  ...
  describe "hook_execution_order" do
    test "Test that multiple hooks of the same type execute in registration order" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/hook_execution_order", connect_options: [protocols: [:http1]])
  ...
  describe "multiple_hooks_all_phases" do
    test "Test multiple lifecycle hooks across all five phases for a complete request lifecycle" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/multiple_hooks_all_phases", connect_options: [protocols: [:http1]], json: %{"action" => "update_profile", "user_id" => "user-123"}, headers: [{"Authorization", "Bearer valid-token-12345"}, {"Content-Type", "application/json"}])
      assert response.status == 200
  ...
  describe "onerror_error_logging" do
    test "Test onError hook that logs server errors and formats error responses" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/onerror_error_logging", connect_options: [protocols: [:http1]])
  ...
  describe "onrequest_request_logging" do
    test "Test onRequest hook that logs incoming requests and adds a request ID" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/onrequest_request_logging", connect_options: [protocols: [:http1]])
      assert response.status == 200
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "x-request-id", do: List.first(List.wrap(v)) end) == ".*"
      body_decoded = if is_binary(response.body), do: Jason.decode!(response.body), else: response.body
  ...
  describe "onresponse_response_timing" do
    test "Test onResponse hook that adds timing information to response headers" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/onresponse_response_timing", connect_options: [protocols: [:http1]])
      assert response.status == 200
      assert Enum.find_value(response.headers, fn {k, v} -> if String.downcase(k) == "x-response-time", do: List.first(List.wrap(v)) end) == ".*ms"
      body_decoded = if is_binary(response.body), do: Jason.decode!(response.body), else: response.body
  ...
  describe "onresponse_security_headers" do
    test "Test onResponse hook that adds security headers to all responses" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/onresponse_security_headers", connect_options: [protocols: [:http1]])
      assert response.status == 200
  ...
  describe "prehandler_authentication_failed_short_circuit" do
    test "Test preHandler hook that short-circuits on invalid authentication" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/prehandler_authentication_failed_short_circuit", connect_options: [protocols: [:http1]], headers: [{"Authorization", "Bearer invalid-token"}])
  ...
  describe "prehandler_authentication_success" do
    test "Test preHandler hook that validates authentication tokens" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/prehandler_authentication_success", connect_options: [protocols: [:http1]], headers: [{"Authorization", "Bearer valid-token-12345"}])
      assert response.status == 200
      body_decoded = if is_binary(response.body), do: Jason.decode!(response.body), else: response.body
  ...
  describe "prehandler_authorization_check" do
    test "Test preHandler hook for role-based authorization after authentication" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/prehandler_authorization_check", connect_options: [protocols: [:http1]], headers: [{"Authorization", "Bearer admin-token-67890"}])
      assert response.status == 200
      body_decoded = if is_binary(response.body), do: Jason.decode!(response.body), else: response.body
  ...
  describe "prehandler_authorization_forbidden_short_circuit" do
    test "Test preHandler hook that denies access for insufficient permissions" do
      {:ok, response} = Req.get(url: "#{mock_server_url()}/fixtures/prehandler_authorization_forbidden_short_circuit", connect_options: [protocols: [:http1]], headers: [{"Authorization", "Bearer user-token-11111"}])
  ...
  describe "prevalidation_rate_limit_exceeded_short_circuit" do
    test "Test preValidation hook that short-circuits when rate limit is exceeded" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/prevalidation_rate_limit_exceeded_short_circuit", connect_options: [protocols: [:http1]], json: %{"data" => "test"}, headers: [{"Content-Type", "application/json"}])
  ...
  describe "prevalidation_rate_limiting" do
    test "Test preValidation hook that implements rate limiting before validation" do
      {:ok, response} = Req.post(url: "#{mock_server_url()}/fixtures/prevalidation_rate_limiting", connect_options: [protocols: [:http1]], json: %{"data" => "test"}, headers: [{"Content-Type", "application/json"}])
      assert response.status == 200
      body_decoded = if is_binary(response.body), do: Jason.decode!(response.body), else: response.body
