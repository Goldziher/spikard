...
# alef:hash:9e7cb688d152827b4a92feddcfb221990bd8a46b8338280defba99d205e4752f
...
defmodule E2e.GraphqlSchemaTest do
  ...
  describe "graphql_authorization_error_variant" do
    test "POST /graphql - Tests GraphQL authorization error when access denied" do
      {:ok, response} = Req.post(client(), url: "/graphql", json: %{"query" => "{ admin { id } }"}, headers: [{"Content-Type", "application/json"}, {"Authorization", "Bearer invalid"}])
  ...
  describe "graphql_custom_endpoint_configuration" do
    test "POST /api/v2/query - Tests GraphQL route configured at custom endpoint" do
      {:ok, response} = Req.post(client(), url: "/api/v2/query", json: %{"query" => "{ user { id } }"}, headers: [{"Content-Type", "application/json"}])
      assert response.status == 200
  ...
  describe "graphql_depth_limit_exceeded" do
    test "POST /graphql - Tests GraphQL query depth validation rejects deeply nested queries" do
      {:ok, response} = Req.post(client(), url: "/graphql", json: %{"query" => "{ user { posts { comments { author { posts { title } } } } } }"}, headers: [{"Content-Type", "application/json"}])
      assert response.status == 200
  ...
  describe "graphql_introspection_disabled" do
    test "POST /graphql - Tests GraphQL introspection returns error when disabled" do
      {:ok, response} = Req.post(client(), url: "/graphql", json: %{"query" => "{ __schema { types { name } } }"}, headers: [{"Content-Type", "application/json"}])
      assert response.status == 200
