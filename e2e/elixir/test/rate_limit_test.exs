...
# alef:hash:f780a5eebba9ffe3a87e257ab088484841b9385e198b57ccc60525b1274872d7
...
defmodule E2e.RateLimitTest do
  ...
  describe "rate_limit_below_threshold_succeeds" do
    test "Verifies that requests below the configured rate limit are served normally." do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/rate_limit_below_threshold_succeeds",
          finch: AlefE2EFinch
        )
  ...
  describe "rate_limit_burst_setting_allows_spike" do
    test "Tests burst setting allows request spike beyond per_second rate" do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/rate_limit_burst_setting_allows_spike",
          finch: AlefE2EFinch
        )
  ...
  describe "rate_limit_exceeded_returns_429" do
    test "Sends sequential requests until the configured limit is exceeded and validates the 429 response." do
      {:ok, response} =
        Req.get(url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/rate_limit_exceeded_returns_429", finch: AlefE2EFinch)
  ...
  describe "rate_limit_ip_based_tracking" do
    test "Tests rate limiting is applied per IP address" do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/rate_limit_ip_based_tracking",
          finch: AlefE2EFinch,
  ...
  describe "rate_limit_per_second_setting_10_requests" do
    test "Tests rate limit allows up to 10 requests per second" do
      {:ok, response} =
        Req.get(
          url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/rate_limit_per_second_setting_10_requests",
          finch: AlefE2EFinch
        )
