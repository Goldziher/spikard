// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:f8c5835aeababf61a9726acee66a21ca6c464558a0c40c6be98c94ac09b9535c
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";

async function _alefE2eDecompressAndParseJson(response: Response): Promise<unknown> {
  const contentEncoding = response.headers.get("content-encoding");
  let bytes = new Uint8Array(await response.arrayBuffer());

  // The runtime's own fetch (undici on Node, and the browser equivalent) decodes
  // `content-encoding` transparently and leaves the header in place, so by the time the
  // body reaches here it is usually already plain text. Decoding unconditionally on the
  // header alone therefore fails ("incorrect header check") against a server that really
  // did compress. Decode only when the payload still carries the encoding: gzip announces
  // itself with the 0x1f8b magic pair; brotli has no magic, so it is attempted and the
  // already-decoded case falls back to the bytes as received.
  if (contentEncoding === "gzip" && bytes.length >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b) {
    const { gunzipSync } = await import("zlib");
    bytes = new Uint8Array(gunzipSync(bytes));
  } else if (contentEncoding === "br") {
    const { brotliDecompressSync } = await import("zlib");
    try {
      bytes = new Uint8Array(brotliDecompressSync(bytes));
    } catch {
      // Already decoded by the runtime.
    }
  }

  const text = new TextDecoder().decode(bytes);
  return JSON.parse(text);
}

function _alefE2eText(value: unknown): string {
  return value == null ? "" : String(value);
}

function _alefE2eItemTexts(item: unknown): string[] {
  if (item == null || typeof item !== "object") {
    return [_alefE2eText(item)];
  }
  const record = item as Record<string, unknown>;
  const itemsText = Array.isArray(record.items) ? record.items.map(_alefE2eText).join(" ") : "";
  return [
    _alefE2eText(item),
    _alefE2eText(record.kind),
    _alefE2eText(record.name),
    _alefE2eText(record.source),
    _alefE2eText(record.alias),
    _alefE2eText(record.text),
    _alefE2eText(record.signature),
    itemsText,
  ];
}

function _alefE2eFormatMetadataDisplay(fm: unknown): string {
  if (fm == null) return "";
  if (typeof fm !== "object") return String(fm);
  const record = fm as Record<string, unknown>;
  const formatType = record.format_type;

  // FormatMetadata is a tagged union: { format_type: 'image', image: { format: 'PNG', ... }, ... }
  // Extract the display string based on the variant type
  if (formatType === "image" && typeof record.image === "object") {
    const imageData = record.image as Record<string, unknown>;
    if (typeof imageData.format === "string") return imageData.format;
  }

  // Fallback: return format_type variant name
  if (typeof record.format_type === "string") return record.format_type;
  return "";
}

describe("graphql_operations", () => {
  it("graphql_field_level_error_partial_data: Tests that a field-level resolver error returns partial data alongside an errors array", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_field_level_error_partial_data/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: '{ user(id: "missing-user") { id name avatar } status }' }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        status: "ok",
        user: null,
      },
      errors: [{ locations: [{ column: 3, line: 1 }], message: "User not found", path: ["user"] }],
    });
  });

  it("graphql_introspection_query: Tests that the __schema introspection query returns the schema's query root type name via real introspection execution", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_introspection_query/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: "{ __schema { queryType { name } } }" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        __schema: {
          queryType: {
            name: "Query",
          },
        },
      },
    });
  });

  it("graphql_invalid_syntax_rejected: Tests that a query with invalid GraphQL syntax returns a syntax error in the errors array", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_invalid_syntax_rejected/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: "{ hello { this is not valid syntax {{{" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: null,
      errors: [{ message: "<<present>>" }],
    });
  });

  it("graphql_multi_operation_with_operation_name: Tests executing a specific named operation from a document containing multiple operations", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_multi_operation_with_operation_name/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ operationName: "Version", query: "query Ping { ping } query Version { version }" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        version: "1.0.0",
      },
    });
  });

  it("graphql_mutation_create_object: Tests GraphQL mutation with an input type that returns the newly created object", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_mutation_create_object/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        query: "mutation CreateUser($input: CreateUserInput!) { createUser(input: $input) { id name email } }",
        variables: { input: { email: "carol@example.com", name: "Carol" } },
      }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        createUser: {
          email: "carol@example.com",
          id: "new-user-id",
          name: "Carol",
        },
      },
    });
  });

  it("graphql_ops_complexity_limit_exceeded: Tests that a query exceeding the configured complexity limit returns an error", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_ops_complexity_limit_exceeded/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        query: "{ users { id friends { id friends { id friends { id friends { id friends { id } } } } } } }",
      }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: null,
      errors: [{ message: "Query is too complex." }],
    });
  });

  it("graphql_ops_depth_limit_exceeded: Tests that a query exceeding the configured depth limit returns an error", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_ops_depth_limit_exceeded/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: "{ a { b { c { d { e } } } } }" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: null,
      errors: [{ message: "Query is nested too deep." }],
    });
  });

  it("graphql_query_nested_fields: Tests GraphQL query selecting nested fields at depth 2-3", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_query_nested_fields/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        query: '{ order(id: "ORD-1") { id customer { name address { city } } items { sku quantity } } }',
      }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        order: {
          customer: {
            address: {
              city: "Springfield",
            },
            name: "Bob",
          },
          id: "ORD-1",
          items: [{ quantity: 2, sku: "WIDGET-A" }],
        },
      },
    });
  });

  it("graphql_query_with_variables: Tests GraphQL query that uses variables passed in the request body", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_query_with_variables/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        query: "query GetUser($id: ID!) { user(id: $id) { id name } }",
        variables: { id: "user-42" },
      }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        user: {
          id: "user-42",
          name: "Alice",
        },
      },
    });
  });

  it("graphql_simple_query: Tests simple GraphQL query execution returning data from the root query type", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_simple_query/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: "{ hello }" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: {
        hello: "world",
      },
    });
  });

  it("graphql_subscription_declaration: Tests that a subscription operation type is accepted by SDL/schema validation, and that executing it over the plain-HTTP (non-streaming) transport is rejected the way real GraphQL engines reject subscriptions on transports that cannot stream events", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_subscription_declaration/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        query: "subscription OnMessageAdded($roomId: ID!) { messageAdded(roomId: $roomId) { id text authorId } }",
        variables: { roomId: "room-1" },
      }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: null,
      errors: [{ message: "Subscriptions are not supported on this transport." }],
    });
  });

  it("graphql_undefined_field_error: Tests that querying a field that does not exist in the schema returns a validation error", async () => {
    const sutUrl = process.env.SUT_URL || "http://127.0.0.1:8001";
    const url = `${sutUrl}/fixtures/graphql_undefined_field_error/graphql`;
    const response = await fetch(url, {
      method: "POST",
      redirect: "manual",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query: "{ hello nonExistentField }" }),
    });
    expect(response.status).toBe(200);
    const data = await _alefE2eDecompressAndParseJson(response);
    expect(data).toEqual({
      data: null,
      errors: [{ locations: [{ column: 9, line: 1 }], message: 'Unknown field "nonExistentField" on type "Query".' }],
    });
  });
});
