...
# alef:hash:50871e2778381e18532f4302066cc685ac90ad6b2ea22ef389e2417f84137fac
...
def test_upload_file_as_bytes() -> None:
    ...
    assert data == {"as_bytes": True, "filename": "binary.bin", "size": 11}
...
def test_upload_file_basic_success() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"filename": "test.txt", "size": 13}
...
def test_upload_file_content_type_or_default() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"content_type": "application/octet-stream", "filename": "unknown.xyz"}
...
def test_upload_file_read_to_string() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"as_string": True, "content": "Hello, World!", "filename": "text.txt"}
...
def test_upload_file_with_custom_content_type() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"content_type": "application/json", "filename": "data.json", "size": 16}
...
def test_upload_file_with_form_fields() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"description": "Important document", "filename": "document.pdf", "tags_count": 2}
...
def test_upload_file_with_gzip_content_encoding() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"content_encoding": "gzip", "filename": "data.gz"}
...
def test_upload_multiple_files() -> None:
    ...
    assert status_code == 200
    data = json.loads(resp_body)
    assert data == {"total_size": 18, "uploaded_files": 2}
