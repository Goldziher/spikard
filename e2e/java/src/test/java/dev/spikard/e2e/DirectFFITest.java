...
public class DirectFFITest {
  public static void main(String[] args) throws Throwable {
    ...
    System.out.println(
        "Testing direct FFI call to spikard_server_config_from_json");

    // First, test with FULL JSON (all fields)
    String fullJson =
        "{\"host\": \"127.0.0.1\", \"port\": 8000, \"workers\": 1, "
        + "\"enable_request_id\": false, \"max_body_size\": null, "
        + "\"request_timeout\": null, \"compression\": null, \"rate_limit\": "
        + "null, \"jwt_auth\": null, \"api_key_auth\": null, \"static_files\": "
        + "[], \"graceful_shutdown\": true, \"shutdown_timeout\": 30, "
        + "\"asyncapi\": null, \"openapi\": null, \"jsonrpc\": null, \"grpc\": "
        + "null, \"background_tasks\": {\"max_concurrent\": 10, "
        + "\"max_queue_size\": 1000, \"shutdown_timeout\": 30}, "
    ...
    MemorySegment addr =
        lookup.find("spikard_server_config_from_json")
    ...
    System.out.println("Result for FULL JSON: " +
                       (result == null || result.address() == 0
                            ? "NULL"
                            : "0x" + Long.toHexString(result.address())));
    ...
    System.out.println("Result for PARTIAL JSON: " +
                       (result == null || result.address() == 0
                            ? "NULL"
                            : "0x" + Long.toHexString(result.address())));
    ...
    if (result == null || result.address() == 0) {
      MemorySegment errorAddr =
          lookup.find("spikard_last_error_code")
              .or(() -> lookup.find("_spikard_last_error_code"))
              .orElseThrow();
      FunctionDescriptor errorDesc =
