...
// alef:hash:49c66106c44b06afe4e1fb50a3e54365f759c1ee844a37124f4b092e91cd8325
...
$repoRoot = __DIR__ . '/../../';
$localExtCandidates = [
    $repoRoot . 'target/release/libspikard_php' . $extSuffix,
    $repoRoot . 'crates/spikard-php/target/release/libspikard_php' . $extSuffix,
];
...
if (!file_exists($extPath)) {
    fwrite(STDERR, "error: no spikard_php PHP extension build found.\n");
    foreach ($localExtCandidates as $candidate) {
    ...
    fwrite(STDERR, "  cargo build --release -p spikard-php\n");
    exit(1);
}
...
$php = PHP_BINARY;
$isolatedIni = alef_build_isolated_ini($php, 'spikard_php');
...
$process = proc_open(
    ...
    $pipes,
    null,
    ['ALEF_PREFLIGHT_EXT_NAME' => 'spikard_php'] + $_ENV,
    ['bypass_shell' => true],
);
if ($process === false) {
    fwrite(STDERR, "error: failed to spawn PHP preflight check for the spikard_php extension.\n");
    exit(1);
}
...
if (stripos($preflightStderr, 'already loaded') !== false) {
    fwrite(STDERR, "error: the spikard_php extension is already loaded from an ambient php.ini\n");
...
if ($preflightExit !== 0 || $preflightStdout === '') {
    fwrite(STDERR, "error: the spikard_php extension did not load from $extPath.\n");
...
if ($loadedVersion !== '0.17.1') {
    fwrite(STDERR, "error: loaded spikard_php extension version mismatch.\n");
    fwrite(STDERR, "  extension at $extPath reports version: $loadedVersion\n");
    fwrite(STDERR, "  expected version: 0.17.1\n");
