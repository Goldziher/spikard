...
// alef:hash:1aac91ae773c96f93940a972ba4c61baab09d11e637231162535a77d1cdb7ad2
...
final class RateLimitTest extends TestCase
{
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/rate_limit_below_threshold_succeeds");
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/rate_limit_burst_setting_allows_spike");
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/rate_limit_exceeded_returns_429");
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/rate_limit_ip_based_tracking", [
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/rate_limit_per_second_setting_10_requests");
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals(["status" => "ok"], $body);
    }
