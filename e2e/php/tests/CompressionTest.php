...
// alef:hash:d436e2f08c904790656275049ec02ac870f53b8c9e33c2928c2acc22116b74c6
...
final class CompressionTest extends TestCase
{
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/compression_brotli_only", [
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/compression_gzip_applied", [
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/compression_min_size_threshold_exact_boundary", [
            'headers' => ["Accept-Encoding" => "gzip"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/compression_payload_below_min_size_is_not_compressed", [
            'headers' => ["Accept-Encoding" => "gzip"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    {
        $response = $this->httpClient->request('GET', "/fixtures/compression_quality_level_9", [
            'headers' => ["Accept-Encoding" => "gzip"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
