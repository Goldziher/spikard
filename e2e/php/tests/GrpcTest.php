...
// alef:hash:b041f8551c4ab0cd25c9438ffec8cb473f4e19ce9da43fb2ec2d326bcc756d77
...
final class GrpcTest extends TestCase
{
    ...
    public function test_grpc_call_with_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/fixtures/grpc_call_with_metadata", [
            'json' => ["id" => "resource-1"],
            'headers' => ["te" => "trailers", "authorization" => "Bearer token123", "x-custom-metadata" => "value123", "Content-Type" => "application/grpc"],
        ]);
    ...
    public function test_grpc_config_compression_enabled(): void
    {
        $response = $this->httpClient->request('POST', "/fixtures/grpc_config_compression_enabled", [
            ...
            'headers' => ["grpc-encoding" => "gzip", "te" => "trailers", "Content-Type" => "application/grpc"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    public function test_grpc_request_with_custom_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/fixtures/grpc_request_with_custom_metadata", [
            ...
            'headers' => ["x-trace-id" => "trace-abc123", "te" => "trailers", "x-user-id" => "user-42", "Content-Type" => "application/grpc", "x-request-id" => "req-12345"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
