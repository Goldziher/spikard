...
// alef:hash:4fa7ee922306a4741e8287d69995d341592fb53cba696a91d7ce89aed5032264
...
final class GrpcTest extends TestCase
{
    ...
    public function test_grpc_call_with_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/api.Service/GetData", [
            'json' => ["id" => "resource-1"],
            'headers' => ["x-custom-metadata" => "value123", "te" => "trailers", "authorization" => "Bearer token123", "Content-Type" => "application/grpc"],
        ]);
    ...
    public function test_grpc_request_with_custom_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/api.Service/ProcessWithMetadata", [
            ...
            'headers' => ["x-request-id" => "req-12345", "x-user-id" => "user-42", "te" => "trailers", "x-trace-id" => "trace-abc123", "Content-Type" => "application/grpc"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
