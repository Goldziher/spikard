...
// alef:hash:b37e5bb10712308464cf18ef6ad0397978daad6046819e16ccbf5bd4c0991903
...
final class GrpcTest extends TestCase
{
    ...
    public function test_grpc_deadline_exceeded(): void
    {
        $response = $this->httpClient->request('POST', "/fixtures/grpc_deadline_exceeded", [
            ...
            'headers' => ["te" => "trailers", "grpc-timeout" => "100m", "Content-Type" => "application/grpc"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
    ...
    public function test_grpc_request_with_custom_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/fixtures/grpc_request_with_custom_metadata", [
            ...
            'headers' => ["Content-Type" => "application/grpc", "te" => "trailers", "x-user-id" => "user-42", "x-request-id" => "req-12345", "x-trace-id" => "trace-abc123"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
