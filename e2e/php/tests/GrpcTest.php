...
// alef:hash:1bb991367a1ff9e712531d21d4af5379f23cfe7f136dab9aa74c017768089566
...
final class GrpcTest extends TestCase
{
    ...
    public function test_grpc_call_with_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/api.Service/GetData", [
            'json' => ["id" => "resource-1"],
            'headers' => ["authorization" => "Bearer token123", "x-custom-metadata" => "value123", "te" => "trailers", "Content-Type" => "application/grpc"],
        ]);
    ...
    public function test_grpc_config_compression_enabled(): void
    {
        $response = $this->httpClient->request('POST', "/api.Service/GetCompressedData", [
            ...
            'headers' => ["te" => "trailers", "grpc-encoding" => "gzip", "Content-Type" => "application/grpc"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals("application/grpc", $response->getHeaderLine("content-type"));
    ...
    public function test_grpc_request_with_custom_metadata(): void
    {
        $response = $this->httpClient->request('POST', "/api.Service/ProcessWithMetadata", [
            ...
            'headers' => ["Content-Type" => "application/grpc", "te" => "trailers", "x-user-id" => "user-42", "x-trace-id" => "trace-abc123", "x-request-id" => "req-12345"],
        ]);
        $body = json_decode((string) $response->getBody(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $response->getStatusCode());
