...
// alef:hash:05118db7a4d1def0dd1fb1485e8a2d8a8fec31258e66c7a08806003535bf2ff9
 ...
 *
 * Per-route middleware is passed as a trailing options array keyed by the
 * RouteBuilder setter names:
...
final class App
{
     ...
     *
     * @param array<string, mixed> $options  Per-route middleware options.
     */
    public function get(string $path, callable $handler, array $options = []): self
    {
        return $this->addRoute('Get', $path, null, $handler, $options);
    }
     ...
     * @param array<string, mixed>  $options   Per-route middleware options.
     */
    public function post(string $path, ?string $dtoClass, callable $handler, array $options = []): self
    {
        return $this->addRoute('Post', $path, $dtoClass, $handler, $options);
    }
     ...
     * @param array<string, mixed>  $options   Per-route middleware options.
     */
    public function put(string $path, ?string $dtoClass, callable $handler, array $options = []): self
    {
        return $this->addRoute('Put', $path, $dtoClass, $handler, $options);
    }
     ...
     * @param array<string, mixed>  $options   Per-route middleware options.
     */
    public function patch(string $path, ?string $dtoClass, callable $handler, array $options = []): self
    {
        return $this->addRoute('Patch', $path, $dtoClass, $handler, $options);
    }
    ...
    {
        return $this->addRoute('Delete', $path, null, $handler, $options);
    }
     ...
     * @param array<string, mixed> $options  Per-route middleware options.
     */
    public function head(string $path, callable $handler, array $options = []): self
    {
        return $this->addRoute('Head', $path, null, $handler, $options);
    }
     ...
     * @param array<string, mixed> $options  Per-route middleware options.
     */
    public function options(string $path, callable $handler, array $options = []): self
    {
        return $this->addRoute('Options', $path, null, $handler, $options);
    }
     ...
     * @param array<string, mixed>  $options   Per-route middleware options.
     */
    public function route(RouteBuilder $builder, ?string $dtoClass, callable $handler, array $options = []): self
    {
        if ($dtoClass !== null) {
            $schema = self::deriveJsonSchema($dtoClass);
            $builder = $builder->requestSchemaJson((string) json_encode($schema, JSON_THROW_ON_ERROR));
        }
        $builder = self::applyRouteOptions($builder, $options);
     ...
     * derived from $dtoClass (if any), apply per-route middleware options, wrap the
     * handler in a request adapter, and append the registration tuple.
     *
     * @param class-string|null     $dtoClass
     * @param array<string, mixed>  $options   Per-route middleware options.
     */
    ...
    /**
     * Apply per-route middleware options to a RouteBuilder.
     *
     * Option keys mirror the RouteBuilder setter names one-for-one, so there is a
     * single spelling to remember. Values are forwarded untouched — the native
     * extension type-checks them and the Rust core owns all validation.
     *
     * Supported keys:
     *   cors, compression, bodyLimit, requestTimeout, rateLimit, requestId,
     *   jwtAuth, apiKeyAuth, authorization, lifecycleHooks, openrpcSpec
     *
     * @param array<string, mixed> $options
     * @throws \InvalidArgumentException  When an option key is not recognised.
     */
    private static function applyRouteOptions(RouteBuilder $builder, array $options): RouteBuilder
    {
        foreach ($options as $key => $value) {
            $builder = match ($key) {
                'cors' => $builder->cors($value),
                'compression' => $builder->compression($value),
                'bodyLimit' => $builder->bodyLimit((int) $value),
                'requestTimeout' => $builder->requestTimeout((int) $value),
                'rateLimit' => $builder->rateLimit($value),
                'requestId' => $builder->requestId((bool) $value),
                'jwtAuth' => $builder->jwtAuth($value),
                'apiKeyAuth' => $builder->apiKeyAuth($value),
                'authorization' => $builder->authorization($value),
                'lifecycleHooks' => $builder->lifecycleHooks($value),
                'openrpcSpec' => $builder->openrpcSpec(
                    is_string($value) ? $value : (string) json_encode($value, JSON_THROW_ON_ERROR),
                ),
                default => throw new \InvalidArgumentException("Unknown route option: {$key}"),
            };
        }
