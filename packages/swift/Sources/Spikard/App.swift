...
// alef:hash:7e0b22ebef977dd46cbb87753acd77b1d891533ff6514cca13bdfdd11ef8370a
...
public final class App {
  ...
  public func get(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    ...
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Get\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
    }
    // Chain the optional `compression` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let compression {
      __builder = RustBridge.routeBuilderCompression(__builder, try compression.intoRust())
    }
  ...
  public func post(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Post\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func put(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Put\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func patch(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Patch\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func delete(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Delete\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func head(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Head\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func options(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Options\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func connect(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Connect\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
  ...
  public func trace(_ handler: @escaping (String) -> String, path: String, cors: CorsConfig? = nil, compression: CompressionConfig? = nil) throws {
    // Construct the wrapper metadata param (e.g. RouteBuilder) from the fixed enum arg
    // and any free args. swift-bridge generates enums as opaque classes — there are no
    // static member constants (e.g. `RustBridge.Method.Get` is invalid). Instead,
    // each opaque enum type has a `<type>FromJson` factory that parses a serde JSON
    // string into an opaque instance. The wrapper constructor factory `routeBuilderNew`
    // is a bridge-declared free function that calls the Rust wrapper's `new` method.
    var __builder = RustBridge.routeBuilderNew(try methodFromJson("\"Trace\""), path)
    // Chain the optional `cors` metadata onto the wrapper before delegating, so
    // the shortcut can express everything the builder can and not just its constructor args.
    if let cors {
      __builder = RustBridge.routeBuilderCors(__builder, try cors.intoRust())
