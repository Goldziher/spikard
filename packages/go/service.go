...
// WebSocket route registration methods.
...
func (s *App) WebSocket(path string, handler func(*WebSocketConnection) error) error {
	if s.owner == nil {
		return errors.New("service is closed")
	}
...
// Server-Sent Events route registration methods.

// SSE registers an SSE event producer at the given path.
// The producer yields SseEvent items from a channel.
func (s *App) SSE(path string, producer func() <-chan interface{}) error {
	if s.owner == nil {
		return errors.New("service is closed")
	}
...
func PathParam(r *http.Request, name string) string {
	...
	// This is a placeholder; actual implementation depends on chi integration.
