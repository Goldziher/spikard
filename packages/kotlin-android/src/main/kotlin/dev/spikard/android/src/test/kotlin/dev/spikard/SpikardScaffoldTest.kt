...
class SpikardScaffoldTest {
    // Round-trips the generated `BackgroundTaskConfig` data class through the Jackson mapping the
    ...
    @Test
    fun backgroundTaskConfigRoundTripsThroughItsGeneratedJsonMapping() {
        val original = BackgroundTaskConfig(true, 1L, 1L, 1L)
        val mapper = jacksonObjectMapper()
        val json = mapper.writeValueAsString(original)
        assertEquals(original, mapper.readValue(json, BackgroundTaskConfig::class.java))
    }
}
