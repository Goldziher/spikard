...
// alef:hash:57c086e531ed43ad71d1322a4fe14e71e482240197175c8b083f5698ae85bde7
...
tasks.register("buildHostJni", Exec::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/spikard-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException(
                "Cannot locate crates/spikard-jni/Cargo.toml from $projectDir; " +
...
tasks.register("copyHostJni", Copy::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/spikard-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException("Cannot locate crates/spikard-jni/Cargo.toml from $projectDir")
...
tasks.register("validateJniLibsForRelease") {
    doFirst {
        ...
        if (releaseAssemble) {
            ...
            // lib*_ffi.so instead of libspikard_jni.so) passes the emptiness
            // check above but fails at runtime with UnsatisfiedLinkError, because
            // System.loadLibrary("spikard_jni") needs the JNI entry points.
            val expectedJniLib = "libspikard_jni.so"
            ...
            if (abisMissingJniLib.isNotEmpty()) {
                throw GradleException(
                    ...
                    "export the JNI symbols System.loadLibrary(\"spikard_jni\") requires and " +
