...
pub fn build(b: *std.Build) void {
    ...
    const ffi_path = b.option([]const u8, "ffi_path", "Path to directory containing libspikard_ffi.{dylib,so,dll,a}") orelse "../../target/release";

    const ffi_include = b.option([]const u8, "ffi_include_path", "Path to directory containing the FFI C header") orelse "../../crates/spikard-ffi/include";

    const module = b.addModule("spikard", .{
