...
pub const FieldErrorSpec = struct {
    ...
    pub fn free(self: *FieldErrorSpec) void {
    c.spikard_field_error_spec_free(@as(*c.SPIKARDFieldErrorSpec, @ptrCast(self._handle)));
    }
};
...
pub const DynamicSchemaConfig = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *DynamicSchemaConfig) void {
    c.spikard_dynamic_schema_config_free(@as(*c.SPIKARDDynamicSchemaConfig, @ptrCast(self._handle)));
    }
};
...
pub const RouteBuilder = struct {
    ...
    pub fn body_limit(self: *RouteBuilder, max_bytes: u64) RouteBuilder {
    const _result = c.spikard_route_builder_body_limit(@as(*c.SPIKARDRouteBuilder, @ptrCast(self._handle)), max_bytes);
        return RouteBuilder{ ._handle = _result.? };
    }

    /// Attach a per-route request timeout in seconds, overriding the server-global default.
    pub fn request_timeout(self: *RouteBuilder, seconds: u64) RouteBuilder {
    const _result = c.spikard_route_builder_request_timeout(@as(*c.SPIKARDRouteBuilder, @ptrCast(self._handle)), seconds);
        return RouteBuilder{ ._handle = _result.? };
    }
