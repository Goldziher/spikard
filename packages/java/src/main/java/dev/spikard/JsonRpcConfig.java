// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:7a0aa20e22e9cfe1e6106550f28dc9facd94cc2e9fe13be44c3311b70a64f1f3
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
package dev.spikard;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import org.jspecify.annotations.Nullable;

/**
 * JSON-RPC server configuration
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = JsonRpcConfig.Builder.class)
public record
JsonRpcConfig(@Nullable @JsonProperty("enabled") Boolean enabled,
              @Nullable @JsonProperty("endpoint_path") String endpointPath,
              @Nullable @JsonProperty("enable_batch") Boolean enableBatch,
              @Nullable @JsonProperty("max_batch_size") Long maxBatchSize) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }
  public JsonRpcConfig {
    if (enabled == null) {
      enabled = true;
    }
  }

  // CPD-OFF
  /** Jackson builder for JsonRpcConfig deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    @Nullable private Boolean enabled;
    @JsonProperty("endpoint_path") @Nullable private String endpointPath;
    @JsonProperty("enable_batch") @Nullable private Boolean enableBatch;
    @JsonProperty("max_batch_size") @Nullable private Long maxBatchSize;

    /** Sets the enabled field. */
    @JsonProperty("enabled")
    public Builder withEnabled(final @Nullable Boolean value) {
      this.enabled = value;
      return this;
    }

    /** Sets the endpointPath field. */
    @JsonProperty("endpoint_path")
    public Builder withEndpointPath(final @Nullable String value) {
      this.endpointPath = value;
      return this;
    }

    /** Sets the enableBatch field. */
    @JsonProperty("enable_batch")
    public Builder withEnableBatch(final @Nullable Boolean value) {
      this.enableBatch = value;
      return this;
    }

    /** Sets the maxBatchSize field. */
    @JsonProperty("max_batch_size")
    public Builder withMaxBatchSize(final @Nullable Long value) {
      this.maxBatchSize = value;
      return this;
    }

    /** Constructs a JsonRpcConfig instance from the builder's current state. */
    public JsonRpcConfig build() {
      return new JsonRpcConfig(enabled, endpointPath, enableBatch,
                               maxBatchSize);
    }
  }
  // CPD-ON
}
