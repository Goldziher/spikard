...
public class App implements AutoCloseable {
  ...
  public int get(String path, Callable handler) {
    try {
      ...
      // Allocate path string in the shared arena
      MemorySegment pathSegment = arena.allocateFrom(path);

      // invokeExact is signature-polymorphic: the call-site argument types must
      // match the handle's descriptor exactly. Pass the segments directly rather
      // than boxing into Object[] (which would make the call site (Object[])int).
      return (int) varHandle.invokeExact(
          ownerHandle, // owner
          upcallStub, // callback
          MemorySegment.NULL, // context: null pointer
          pathSegment // path: *const c_char
          );
    } catch (Throwable e) {
