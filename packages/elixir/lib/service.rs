...
// alef:hash:4f9fe15600057b14030a1d9f8badc68832ff0b837af8c09352f7aded8886e699
...
pub fn app_run(registrations: rustler::Term<'_>) -> NifResult<Atom> {
    ...
    for reg_entry in registration_list {
        // `let ... else { continue; }` rather than `if let ... { .. }`: the body is a
        // single `if method_name == ..` chain, so the nested form is exactly what
        // `clippy::collapsible_if` rejects -- and a generated crate is linted like any other.
        let Ok((method_name, metadata, handler_pid)) =
            reg_entry.decode::<(String, rustler::Term<'_>, rustler::LocalPid)>()
        else {
            continue;
        };
        if method_name == "route"
            && let Ok((builder,)) = metadata.decode::<(rustler::ResourceArc<super::RouteBuilder>,)>()
        {
    ...
    let rt = tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .map_err(|_e| NifError::Atom("runtime_error"))?;
...
pub fn app_get(registrations: rustler::Term<'_>, path: String, handler: rustler::LocalPid) -> NifResult<Atom> {
    ...
    for reg_entry in registration_list {
        // See `service_api_run_nif_header`: the nested `if let` inside `if let` is a
        // `clippy::collapsible_if` failure in the generated crate.
        let Ok((_method, _metadata, handler_pid)) =
            reg_entry.decode::<(String, rustler::Term<'_>, rustler::LocalPid)>()
        else {
            continue;
        };
