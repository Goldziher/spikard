...
/// Create a structured error tuple with a custom reason atom and message.
///
/// This function creates an error tuple in the format `{:error, {:reason, "message"}}`,
/// allowing for consistent error handling across the NIF layer.
///
/// # Arguments
///
/// * `env` - The Rustler environment for term creation
/// * `reason` - The reason atom to include in the error tuple
/// * `message` - The error message string
///
/// # Returns
///
/// An Elixir term representing `{:error, {:reason_atom, "message"}}`
///
/// # Example
///
/// ```ignore
/// let err = struct_error(env, atoms::invalid_port(), "Port must be between 1 and 65535");
/// // Returns: {:error, {:invalid_port, "Port must be between 1 and 65535"}}
/// ```
pub fn struct_error<'a>(env: Env<'a>, reason: rustler::types::atom::Atom, message: &str) -> Term<'a> {
    (atoms::error(), (reason, message)).encode(env)
}
