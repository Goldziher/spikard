...
# alef:hash:d7562a9c9787c82bb88f62c5ef4b610226c57de7fd0c6b0e15c139aba822f8f9
...
import threading
import types
from typing import TYPE_CHECKING, Any, TypedDict, Union, get_args, get_origin, get_type_hints

from spikard import _spikard as _native
...
if TYPE_CHECKING:
    ...
    from spikard._spikard import ApiKeyAuthConfig, JwtAuthConfig
    from spikard.options import (
        AuthorizationConfig,
        CompressionConfig,
        CorsConfig,
...
__all__ = ["App", "RouteMiddleware"]


class RouteMiddleware(TypedDict, total=False):
    """Per-route middleware options, one key per ``RouteBuilder`` setter.

    Every value is forwarded verbatim to the Rust core, which owns the actual middleware
    behaviour; this layer only converts the public ``spikard.options`` dataclasses into the
    native configuration objects the builder accepts.
    """
    ...
    openrpc_spec: str | dict[str, Any]


# Middleware keys whose value is forwarded as-is to the identically named ``RouteBuilder``
# setter. ``openrpc_spec`` is handled separately because it also accepts a mapping.
_ROUTE_MIDDLEWARE_SETTERS = (
    "cors",
    "compression",
    "body_limit",
    "request_timeout",
...
# ``TypedDict`` is erased at runtime, so a mistyped key would otherwise be silently dropped and
# the route would serve without the middleware the user believed they had configured. ~keep
_ROUTE_MIDDLEWARE_KEYS = frozenset(_ROUTE_MIDDLEWARE_SETTERS) | {"openrpc_spec"}
...
def _native_config(value: Any) -> Any:
    """Convert a public ``spikard.options`` config dataclass into its native counterpart.

    ``spikard.CorsConfig`` and friends are plain dataclasses, while the ``RouteBuilder`` setters
    accept only the PyO3 classes of the same name exported by ``spikard._spikard``. Values that
    are already native (or have no native twin) are returned untouched, so a user may pass
    either form. ~keep
    """
    if isinstance(value, type) or not dataclasses.is_dataclass(value):
        return value
    native_type = getattr(_native, type(value).__name__, None)
    if native_type is None:
        return value
    return native_type(**{field.name: _native_field(getattr(value, field.name)) for field in dataclasses.fields(value)})


def _native_field(value: Any) -> Any:
    """Convert one dataclass field value, descending into lists of nested config dataclasses."""
    if isinstance(value, list):
        return [_native_config(item) for item in value]
    return _native_config(value)
...
class App:
    ...
    def add_route(
        ...
        handler: Callable[..., Any],
        *,
        middleware: RouteMiddleware | None = None,
    ) -> Callable[..., Any]:
        ...
        auth, lifecycle hooks, body limit, request timeout, request id, OpenRPC spec).
        """
        ...
        if middleware:
            builder = self._apply_middleware(builder, middleware)
    ...
    @staticmethod
    def _apply_middleware(builder: _RouteBuilder, middleware: RouteMiddleware) -> _RouteBuilder:
        """Chain the per-route middleware entries of ``middleware`` onto ``builder``."""
        unknown = set(middleware) - _ROUTE_MIDDLEWARE_KEYS
        if unknown:
            raise ValueError(f"Unsupported route middleware option(s): {', '.join(sorted(unknown))}")

        options: dict[str, Any] = dict(middleware)
        for key in _ROUTE_MIDDLEWARE_SETTERS:
            value = options.get(key)
            if value is None:
                continue
            builder = getattr(builder, key)(_native_config(value))

        spec = options.get("openrpc_spec")
        if spec is not None:
            builder = builder.openrpc_spec(spec if isinstance(spec, str) else json.dumps(spec))

        return builder

    def _method_decorator(
        self, method: str, middleware: RouteMiddleware | None = None
    ) -> Callable[[str], Callable[[Callable[..., Any]], Callable[..., Any]]]:
        def register(path: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
            def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
                return self.add_route(method, path, func, middleware=middleware)
    ...
    def get(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a GET route."""
        return self._method_decorator("GET", middleware)(path)

    def post(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a POST route."""
        return self._method_decorator("POST", middleware)(path)

    def put(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a PUT route."""
        return self._method_decorator("PUT", middleware)(path)

    def patch(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a PATCH route."""
        return self._method_decorator("PATCH", middleware)(path)

    def delete(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a DELETE route."""
        return self._method_decorator("DELETE", middleware)(path)

    def head(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a HEAD route."""
        return self._method_decorator("HEAD", middleware)(path)

    def options(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register an OPTIONS route."""
        return self._method_decorator("OPTIONS", middleware)(path)

    def connect(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a CONNECT route."""
        return self._method_decorator("CONNECT", middleware)(path)

    def trace(
        self, path: str, *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a TRACE route."""
        return self._method_decorator("TRACE", middleware)(path)

    def route(
        self, path: str, method: str = "GET", *, middleware: RouteMiddleware | None = None
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        """Register a route with an explicit HTTP method (defaults to GET)."""
        return self._method_decorator(method, middleware)(path)
