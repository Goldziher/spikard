...
module Spikard
  # HTTP response facade for building and manipulating HTTP responses
  class Response
    ...
    def set_header(name, value)
      @headers[name.to_s.downcase] = value.to_s
    end
  ...
  # Streaming HTTP response wrapper for streaming content delivery
  class StreamingResponse
    ...
    def initialize(stream, status_code: 200, headers: nil)
      ...
      @headers = header_hash.each_with_object({}) do |(key, value), memo|
        memo[String(key).downcase] = String(value)
      end
    end
  end
