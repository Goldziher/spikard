...
// alef:hash:d8e9b6c139f41994a688d23540223bd60afc51257d7fbc05ed32220c102a3201
...
fn call_ruby_proc_with_gvl(
    ...
    unsafe {
        ...
        state.result.unwrap_or_else(|| {
            Err(Box::new(std::io::Error::other("GVL callback failed to set result"))
...
extern "C" fn ruby_proc_gvl_callback(data: *mut std::ffi::c_void) -> *mut std::ffi::c_void {
    ...
    unsafe {
        ...
        let ruby = match Ruby::get() {
            Ok(r) => r,
            Err(_) => {
                state.result = Some(Err(Box::new(std::io::Error::other(
