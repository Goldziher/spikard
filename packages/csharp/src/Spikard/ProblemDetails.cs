...
// alef:hash:9fcd7c7f12c7bcd2a69bf00d9494a2b247ed60bc232e011cacc1eedbc9f9963a
...
public sealed record ProblemDetails {
    ...
    try {
      return JsonSerializer.Deserialize<ProblemDetails>(json, JsonOptions) ??
             throw new SpikardException(
                 $"Failed to parse ProblemDetails from JSON: deserializer returned null");
    } catch (SpikardException) {
      throw;
    } catch (Exception e) {
      throw new SpikardException(
          $"Failed to parse ProblemDetails from JSON: {e.Message}", e);
    }
  }
  ...
  public static ProblemDetails Default() {
    ...
    return JsonSerializer.Deserialize<ProblemDetails>(json ?? "null",
                                                      JsonOptions)!;
  }
  ...
  public ProblemDetails WithDetail(string detail) {
    ...
    try {
      var nativeResult =
          NativeMethods.ProblemDetailsWithDetail(selfHandle, detail);
      var jsonPtr = NativeMethods.ProblemDetailsToJson(nativeResult);
  ...
  public ProblemDetails WithInstance(string instance) {
    ...
    try {
      var nativeResult =
          NativeMethods.ProblemDetailsWithInstance(selfHandle, instance);
      var jsonPtr = NativeMethods.ProblemDetailsToJson(nativeResult);
      var json = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
          jsonPtr);
      NativeMethods.FreeString(jsonPtr);
