...
// alef:hash:bc33ad0330c3a6c55be8de7f3ebc7072896a3bdcf4a99701009364f44c9c4a67
...
public sealed record LifecycleHooksConfig
{
    ...
    public List<LifecycleHookRef> PreValidation { get; init; } = [];

    /// <summary>
    /// Hooks to run in the `pre_handler` phase
    /// </summary>
    [JsonPropertyName("pre_handler")]
    public List<LifecycleHookRef> PreHandler { get; init; } = [];

    /// <summary>
    /// Hooks to run in the `on_response` phase
    /// </summary>
    [JsonPropertyName("on_response")]
    public List<LifecycleHookRef> OnResponse { get; init; } = [];

    /// <summary>
    /// Hooks to run in the `on_error` phase
    /// </summary>
    [JsonPropertyName("on_error")]
    public List<LifecycleHookRef> OnError { get; init; } = [];
    ...
    public static LifecycleHooksConfig FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<LifecycleHooksConfig>(json, JsonOptions)
                ?? throw new SpikardException($"Failed to parse LifecycleHooksConfig from JSON: deserializer returned null");
        }
        ...
        {
            throw new SpikardException($"Failed to parse LifecycleHooksConfig from JSON: {e.Message}", e);
        }
    }
