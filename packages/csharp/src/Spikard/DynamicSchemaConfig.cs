...
public sealed record DynamicSchemaConfig {
  ...
  public List<FieldErrorSpec> FieldErrors {
    get; init;
  } = [];
  ...
  public static DynamicSchemaConfig FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<DynamicSchemaConfig>(json,
                                                             JsonOptions) ??
             throw new SpikardException(
                 $"Failed to parse DynamicSchemaConfig from JSON: deserializer returned null");
    } catch (SpikardException) {
      throw;
    } catch (Exception e) {
      throw new SpikardException(
          $"Failed to parse DynamicSchemaConfig from JSON: {e.Message}", e);
    }
  }
