...
const FRB_INIT_REPLACEMENT: &str = r#"  /// Resolve the prebuilt native library from the environment, the package's bundled
  ...
  /// 4. Versioned user cache populated by `dart run spikard:download_libs`
  ///    (`<cache>/spikard/<version>/<rid>/`), shared with the download script
  ...
  static Future<ExternalLibrary?> _alefResolveExternalLibrary() async {
    try {
      ...
      if (rid != null) {
        final packageRoot =
            await Isolate.resolvePackageUri(Uri.parse('package:spikard/spikard.dart'));
        if (packageRoot != null) {
      ...
      // Versioned user cache populated by `dart run spikard:download_libs`.
