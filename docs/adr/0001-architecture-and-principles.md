...
Spikard is a Rust-first web toolkit that ships comparable APIs across Python, Node/Bun, Ruby, and PHP. We need a durable statement of principles and a concise architecture map that matches the current codebase and bindings.
...
- **Bindings surface idiomatic APIs**: PyO3 (`crates/spikard-py`), napi-rs (`crates/spikard-node`), Magnus (`packages/ruby/ext/spikard_rb/native`), and ext-php-rs (`crates/spikard-php`) expose the same behaviors with language-specific ergonomics (decorators/blocks/functions).
...
- Bindings: `crates/spikard-py`, `crates/spikard-node`, `packages/ruby/ext/spikard_rb/native`, `crates/spikard-php`
