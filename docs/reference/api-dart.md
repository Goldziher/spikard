...
<!-- alef:hash:a9eee9cc241a40ecabb97fe8c385f31f54ced37b15c868edb3a0d1fdd32e2cd1 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v0.17.1</span>
...
```dart
Future<FullSchemaConfig> schemaFull()
```
...
| `maxQueueSize` | `PlatformInt64` | `1024` | Maximum queue size |
| `maxConcurrentTasks` | `PlatformInt64` | `128` | Maximum concurrent tasks |
...
| `minSize` | `PlatformInt64` | `1024` | Minimum response size to compress (bytes) |
...
| `maxComplexity` | `PlatformInt64?` | `null` | Maximum query complexity (`null` = unlimited). |
| `maxDepth` | `PlatformInt64?` | `null` | Maximum query depth (`null` = unlimited). |
...
| `complexityLimit` | `PlatformInt64?` | `null` | Maximum query complexity (None = unlimited) |
| `depthLimit` | `PlatformInt64?` | `null` | Maximum query depth (None = unlimited) |
...
```dart
static Future<GraphQlRouteConfig> create()
```

**Example:**

```dart
final result = GraphQlRouteConfig.create();
```
...
```dart
Future<GraphQlRouteConfig> path(String path)
```
...
```dart
Future<GraphQlRouteConfig> method(String method)
```
...
```dart
Future<GraphQlRouteConfig> enablePlayground(bool enable)
```
...
```dart
Future<GraphQlRouteConfig> description(String description)
```
...
| `maxStreamResponseBytes` | `PlatformInt64?` | `null` | Total byte cap across an entire streaming response. When `Some(n)`, the streaming adapter aborts the stream with `tonic.Status.resource_exhausted` once the cumulative encoded message bytes exceed `n`. The stream yields the error item and then terminates. Per-message cap remains `max_message_size`. This limit applies to server-streaming and bidirectional-streaming RPCs only; unary RPCs are governed solely by `max_message_size`. Default: `null` (unbounded total response size). |
...
| `maxBatchSize` | `PlatformInt64` | `100` | Maximum number of requests in a batch (default: 100) |
...
```dart
static Future<RouteBuilder> create(Method method, String path)
```

**Example:**

```dart
final result = RouteBuilder.create(Method(), "value");
```
...
```dart
Future<RouteBuilder> handlerName(String name)
```
...
```dart
Future<RouteBuilder> requestSchemaJson(String schema)
```
...
```dart
Future<RouteBuilder> responseSchemaJson(String schema)
```
...
```dart
Future<RouteBuilder> paramsSchemaJson(String schema)
```
...
```dart
Future<RouteBuilder> fileParamsJson(String schema)
```
...
```dart
Future<RouteBuilder> cors(CorsConfig cors)
```
...
```dart
Future<RouteBuilder> compression(CompressionConfig compression)
```
...
```dart
Future<RouteBuilder> bodyLimit(PlatformInt64 maxBytes)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `maxBytes` | `PlatformInt64` | Yes | The max bytes |
...
```dart
Future<RouteBuilder> requestTimeout(int seconds)
```
...
```dart
Future<RouteBuilder> rateLimit(RateLimitConfig rateLimit)
```
...
```dart
Future<RouteBuilder> requestId(bool enabled)
```
...
```dart
Future<RouteBuilder> jwtAuth(JwtAuthConfig config)
```
...
```dart
Future<RouteBuilder> apiKeyAuth(ApiKeyAuthConfig config)
```
...
```dart
Future<RouteBuilder> authorization(AuthorizationConfig config)
```
...
```dart
Future<RouteBuilder> lifecycleHooks(LifecycleHooksConfig hooks)
```
...
```dart
Future<RouteBuilder> jsonrpcMethod(JsonRpcMethodInfo info)
```
...
```dart
Future<RouteBuilder> openrpcSpec(String spec)
```
...
```dart
Future<RouteBuilder> handlerDependencies(List<String> dependencies)
```
...
| `workers` | `PlatformInt64` | `1` | Number of Tokio runtime worker threads used by binding-managed server runtimes |
| `enableRequestId` | `bool` | `false` | Enable request ID generation and propagation |
| `maxBodySize` | `PlatformInt64?` | `10485760` | Maximum request body size in bytes (None = unlimited, not recommended) |
...
```dart
Future<ResponseSnapshot> graphqlAt(String endpoint, String query, {String? variables, String? operationName})
```
...
```dart
Future<ResponseSnapshot> graphql(String query, {String? variables, String? operationName})
```
...
```dart
Future<GraphQlSubscriptionSnapshot> graphqlSubscriptionAt(String endpoint, String query, {String? variables, String? operationName})
```
...
```dart
Future<GraphQlSubscriptionSnapshot> graphqlSubscription(String query, {String? variables, String? operationName})
```
...
| `size` | `PlatformInt64?` | `null` | Size of the file in bytes |
