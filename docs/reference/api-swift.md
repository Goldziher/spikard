...
<!-- alef:hash:6e1e0a0793393174fd9e584e5b0adc40e459580f5bbe21fa03033aebf958d58e -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v0.17.0-rc.11</span>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `requiredRoles` | `\[String\]` | `[]` | Roles the authenticated caller must have |
| `requiredScopes` | `\[String\]` | `[]` | OAuth-style scopes the authenticated caller must have |
| `requiredPermissions` | `\[String\]` | `[]` | Fine-grained permissions the authenticated caller must have |
...
| `maxComplexity` | `UInt64?` | `null` | Maximum query complexity (`null` = unlimited). |
| `maxDepth` | `UInt64?` | `null` | Maximum query depth (`null` = unlimited). |
| `fieldErrors` | `\[FieldErrorSpec\]` | `[]` | Field-level errors to inject at specific response paths. |
...
| `dependencies` | `\[String\]` | `[]` | Dependency keys this hook requires (for DI), resolved before the hook runs |
| `config` | `String?` | `null` | Optional free-form configuration passed to the hook (e.g. rate-limit thresholds) |
| `order` | `UInt32?` | `null` | Explicit execution order within the phase, where the corpus states one (`fixtures/lifecycle_hooks.json`'s `hook_execution_order`). Array position already implies an order, so this exists to let a fixture assert ordering rather than rely on it. ~keep |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `onRequest` | `\[LifecycleHookRef\]` | `[]` | Hooks to run in the `on_request` phase |
| `preValidation` | `\[LifecycleHookRef\]` | `[]` | Hooks to run in the `pre_validation` phase |
| `preHandler` | `\[LifecycleHookRef\]` | `[]` | Hooks to run in the `pre_handler` phase |
| `onResponse` | `\[LifecycleHookRef\]` | `[]` | Hooks to run in the `on_response` phase |
| `onError` | `\[LifecycleHookRef\]` | `[]` | Hooks to run in the `on_error` phase |
...
```swift
public func bodyLimit(maxBytes: UInt64) -> RouteBuilder
```

**Example:**

```swift
let result = instance.bodyLimit(42)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `maxBytes` | `UInt64` | Yes | The max bytes |
...
```swift
public func requestTimeout(seconds: UInt64) -> RouteBuilder
```

**Example:**

```swift
let result = instance.requestTimeout(42)
```
...
```swift
public func rateLimit(rateLimit: RateLimitConfig) -> RouteBuilder
```

**Example:**

```swift
let result = instance.rateLimit(RateLimitConfig())
```
...
```swift
public func requestId(enabled: Bool) -> RouteBuilder
```

**Example:**

```swift
let result = instance.requestId(true)
```
...
```swift
public func jwtAuth(config: JwtAuthConfig) -> RouteBuilder
```

**Example:**

```swift
let result = instance.jwtAuth(JwtAuthConfig())
```
...
```swift
public func apiKeyAuth(config: ApiKeyAuthConfig) -> RouteBuilder
```

**Example:**

```swift
let result = instance.apiKeyAuth(ApiKeyAuthConfig())
```
...
```swift
public func authorization(config: AuthorizationConfig) -> RouteBuilder
```

**Example:**

```swift
let result = instance.authorization(AuthorizationConfig())
```
...
```swift
public func lifecycleHooks(hooks: LifecycleHooksConfig) -> RouteBuilder
```

**Example:**

```swift
let result = instance.lifecycleHooks(LifecycleHooksConfig())
```
...
```swift
public func jsonrpcMethod(info: JsonRpcMethodInfo) -> RouteBuilder
```

**Example:**

```swift
let result = instance.jsonrpcMethod(JsonRpcMethodInfo())
```
...
```swift
public func openrpcSpec(spec: String) -> RouteBuilder
```

**Example:**

```swift
let result = instance.openrpcSpec([:])
```
