...
<!-- alef:hash:9be2fc7dbc4f076c701b76443ce3f1bc8033879be1e34e34a834003d74a071bc -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v0.17.1</span>
...
| `maxComplexity` | `UInt?` | `null` | Maximum query complexity (`null` = unlimited). |
| `maxDepth` | `UInt?` | `null` | Maximum query depth (`null` = unlimited). |
...
| `complexityLimit` | `UInt?` | `null` | Maximum query complexity (None = unlimited) |
| `depthLimit` | `UInt?` | `null` | Maximum query depth (None = unlimited) |
...
| `maxStreamResponseBytes` | `UInt?` | `null` | Total byte cap across an entire streaming response. When `Some(n)`, the streaming adapter aborts the stream with `tonic.Status.resource_exhausted` once the cumulative encoded message bytes exceed `n`. The stream yields the error item and then terminates. Per-message cap remains `max_message_size`. This limit applies to server-streaming and bidirectional-streaming RPCs only; unary RPCs are governed solely by `max_message_size`. Default: `null` (unbounded total response size). |
...
| `order` | `UInt32?` | `null` | Explicit execution order within the phase, where the corpus states one (`fixtures/lifecycle_hooks.json`'s `hook_execution_order`). Array position already implies an order, so this exists to let a fixture assert ordering rather than rely on it. |
...
```swift
public func bodyLimit(maxBytes: UInt) -> RouteBuilder
```
...
| `size` | `UInt?` | `null` | Size of the file in bytes |
