...
<!-- alef:hash:72c96cbcc8f7034049142f73e101917be8d3a6bdc8e43d7b22bbe16a146287f0 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Go API Reference <span class="version-badge">v0.17.0-rc.11</span>
...
| `MaxComplexity` | `*int` | `nil` | Maximum query complexity (`nil` = unlimited). |
| `MaxDepth` | `*int` | `nil` | Maximum query depth (`nil` = unlimited). |
...
| `Order` | `*uint32` | `nil` | Explicit execution order within the phase, where the corpus states one (`fixtures/lifecycle_hooks.json`'s `hook_execution_order`). Array position already implies an order, so this exists to let a fixture assert ordering rather than rely on it. ~keep |
...
```go
func (o *RouteBuilder) BodyLimit(maxBytes int) RouteBuilder
```
