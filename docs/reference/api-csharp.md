...
<!-- alef:hash:bd271248e90e34b5a7836b09a85d2f692e457a5b38d74d03a0883fc5566d6847 -->
...
| `Order` | `uint?` | `null` | Explicit execution order within the phase, where the corpus states one (`fixtures/lifecycle_hooks.json`'s `hook_execution_order`). Array position already implies an order, so this exists to let a fixture assert ordering rather than rely on it. ~keep |
