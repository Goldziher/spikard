...
<!-- alef:hash:f65e37fa27387d37e6952d2a97570f0bfe0f62f96153f801ad71dca13e960dda -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## TypeScript API Reference <span class="version-badge">v0.17.0</span>
...
| `maxComplexity` | `number \| null` | `null` | Maximum query complexity (`null` = unlimited). |
| `maxDepth` | `number \| null` | `null` | Maximum query depth (`null` = unlimited). |
...
```typescript
const result = instance.bodyLimit(42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `maxBytes` | `number` | Yes | The max bytes |
...
```typescript
const result = instance.requestTimeout(42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `seconds` | `number` | Yes | The seconds |
...
```typescript
rateLimit(rateLimit: RateLimitConfig): RouteBuilder
```

**Example:**

```typescript
const result = instance.rateLimit(new RateLimitConfig());
```
...
```typescript
const result = instance.requestId(true);
```
...
```typescript
const result = instance.jwtAuth(new JwtAuthConfig());
```
...
```typescript
const result = instance.apiKeyAuth(new ApiKeyAuthConfig());
```
...
```typescript
authorization(config: AuthorizationConfig): RouteBuilder
```

**Example:**

```typescript
const result = instance.authorization(new AuthorizationConfig());
```
...
```typescript
lifecycleHooks(hooks: LifecycleHooksConfig): RouteBuilder
```

**Example:**

```typescript
const result = instance.lifecycleHooks(new LifecycleHooksConfig());
```
...
```typescript
const result = instance.jsonrpcMethod(new JsonRpcMethodInfo());
```
...
```typescript
openrpcSpec(spec: unknown): RouteBuilder
```

**Example:**

```typescript
const result = instance.openrpcSpec({});
```
