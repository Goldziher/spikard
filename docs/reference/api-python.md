...
<!-- alef:hash:a24727fc6300ba4aac893f520f4843ad140fadb128513e83494101e0c07746b4 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Python API Reference <span class="version-badge">v0.17.0</span>
...
```python
result = instance.rate_limit(RateLimitConfig())
```
...
```python
result = instance.jwt_auth(JwtAuthConfig())
```
...
```python
result = instance.api_key_auth(ApiKeyAuthConfig())
```
...
```python
result = instance.lifecycle_hooks(LifecycleHooksConfig())
```
...
```python
result = instance.jsonrpc_method(JsonRpcMethodInfo())
```
...
```python
def openrpc_spec(self, spec: dict[str, Any]) -> RouteBuilder
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `spec` | `dict\[str, Any\]` | Yes | The spec |
...
|-----------|-------------|
| `RouteError(AppError)` | Route registration failed. |
| `ServerError(AppError)` | Server/router construction failed. |
| `DecodeError(AppError)` | Failed to extract DTO from the request context. |
| `GraphQLError(AppError)` | GraphQL route registration failed (e.g. an unrecognized `schema_type`). |
...
| `NotFoundError(GraphQlError)` | Not found error Occurs when a requested resource is not found. |
| `RateLimitExceededError(GraphQlError)` | Rate limit error Occurs when rate limit is exceeded. |
| `InvalidInputError(GraphQlError)` | Invalid input error with validation details Occurs during input validation with detailed error information. |
| `ComplexityLimitExceededError(GraphQlError)` | Query complexity limit exceeded Occurs when a GraphQL query exceeds the configured complexity limit. |
| `DepthLimitExceededError(GraphQlError)` | Query depth limit exceeded Occurs when a GraphQL query exceeds the configured depth limit. |
| `IntrospectionDisabledError(GraphQlError)` | Introspection query rejected because introspection is disabled Occurs when a query selects `__schema` or `__type` while the schema was configured with introspection disabled. |
...
| Exception | Description |
|-----------|-------------|
| `BuildingFailedError(SchemaError)` | Generic schema building error |
| `ValidationError(SchemaError)` | Configuration validation error |
| `ComplexityLimitExceededError(SchemaError)` | Complexity limit exceeded |
| `DepthLimitExceededError(SchemaError)` | Depth limit exceeded |
