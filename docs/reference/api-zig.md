...
<!-- alef:hash:0124f6756d29cfdb74d9fd8f0158907b69051d8efb495aa742b37459b7f89e55 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Zig API Reference <span class="version-badge">v0.17.0-rc.11</span>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `requiredRoles` | `\[\]const \[:0\]const u8` | `[]` | Roles the authenticated caller must have |
| `requiredScopes` | `\[\]const \[:0\]const u8` | `[]` | OAuth-style scopes the authenticated caller must have |
| `requiredPermissions` | `\[\]const \[:0\]const u8` | `[]` | Fine-grained permissions the authenticated caller must have |
...
| `maxComplexity` | `u64?` | `null` | Maximum query complexity (`null` = unlimited). |
| `maxDepth` | `u64?` | `null` | Maximum query depth (`null` = unlimited). |
| `fieldErrors` | `\[\]const FieldErrorSpec` | `[]` | Field-level errors to inject at specific response paths. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `\[:0\]const u8` | — | Dot-separated path to the field that should error. |
| `message` | `\[:0\]const u8` | — | The error message to surface for that field. |
...
| `secret` | `\[:0\]const u8?` | `/* serde(default) */` | Symmetric secret key for JWT verification (HS256, HS384, HS512) |
| `publicKey` | `\[:0\]const u8?` | `/* serde(default) */` | Asymmetric public key for JWT verification (RS256, ES256, etc.) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Registered name of the hook to run, resolved against the server's `LifecycleHooks` |
| `handler` | `\[:0\]const u8` | — | Name of the handler function this hook invokes |
| `dependencies` | `\[\]const \[:0\]const u8` | `[]` | Dependency keys this hook requires (for DI), resolved before the hook runs |
| `config` | `\[:0\]const u8?` | `null` | Optional free-form configuration passed to the hook (e.g. rate-limit thresholds) |
| `order` | `u32?` | `null` | Explicit execution order within the phase, where the corpus states one (`fixtures/lifecycle_hooks.json`'s `hook_execution_order`). Array position already implies an order, so this exists to let a fixture assert ordering rather than rely on it. ~keep |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `onRequest` | `\[\]const LifecycleHookRef` | `[]` | Hooks to run in the `on_request` phase |
| `preValidation` | `\[\]const LifecycleHookRef` | `[]` | Hooks to run in the `pre_validation` phase |
| `preHandler` | `\[\]const LifecycleHookRef` | `[]` | Hooks to run in the `pre_handler` phase |
| `onResponse` | `\[\]const LifecycleHookRef` | `[]` | Hooks to run in the `on_response` phase |
| `onError` | `\[\]const LifecycleHookRef` | `[]` | Hooks to run in the `on_error` phase |
...
```zig
pub fn bodyLimit(self: *const RouteBuilder, max_bytes: u64) RouteBuilder
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `maxBytes` | `u64` | Yes | The max bytes |
...
```zig
pub fn requestTimeout(self: *const RouteBuilder, seconds: u64) RouteBuilder
```
...
```zig
pub fn rateLimit(self: *const RouteBuilder, rate_limit: RateLimitConfig) RouteBuilder
```

**Example:**

```zig
const result = instance.rateLimit(.{});
```
...
```zig
pub fn requestId(self: *const RouteBuilder, enabled: bool) RouteBuilder
```
...
```zig
pub fn jwtAuth(self: *const RouteBuilder, config: JwtAuthConfig) RouteBuilder
```

**Example:**

```zig
const result = instance.jwtAuth(.{});
```
...
```zig
pub fn apiKeyAuth(self: *const RouteBuilder, config: ApiKeyAuthConfig) RouteBuilder
```

**Example:**

```zig
const result = instance.apiKeyAuth(.{});
```
...
```zig
pub fn authorization(self: *const RouteBuilder, config: AuthorizationConfig) RouteBuilder
```
...
```zig
pub fn lifecycleHooks(self: *const RouteBuilder, hooks: LifecycleHooksConfig) RouteBuilder
```

**Example:**

```zig
const result = instance.lifecycleHooks(.{});
```
...
```zig
pub fn jsonrpcMethod(self: *const RouteBuilder, info: JsonRpcMethodInfo) RouteBuilder
```

**Example:**

```zig
const result = instance.jsonrpcMethod(.{});
```
...
```zig
pub fn openrpcSpec(self: *const RouteBuilder, spec: [:0]const u8) RouteBuilder
```
