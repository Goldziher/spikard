...
fn create_handler(ctx: *anyopaque, json_req: [*:0]const u8) callconv(.C) [*:0]u8 {
    _ = ctx;
    _ = json_req;
    // Request body is validated against schema before reaching handler
    // Invalid bodies return 400 with validation errors
...
pub fn main() !void {
    ...
    defer app.deinit();
    _ = app.post(&create_handler, null, "/transactions");
    _ = app.run();
}
```
