...
for bottle in "$artifacts_dir"/spikard-*.bottle.tar.gz; do
  if [ -f "$bottle" ]; then
...
if [ ! -d "$tap_dir" ]; then
  ...
  git clone https://github.com/Goldziher/homebrew-tap.git "$tap_dir"
fi

formula_path="$tap_dir/Formula/spikard.rb"
...
tarball_url="https://github.com/Goldziher/spikard/archive/$tag.tar.gz"
tarball_sha256=$(curl -sL "$tarball_url" | shasum -a 256 | cut -d' ' -f1)
echo "Source tarball SHA256: $tarball_sha256"

bottle_block="  bottle do"
bottle_block+=$'\n'"    root_url \"https://github.com/Goldziher/spikard/releases/download/$tag\""
...
new_formula=$(echo "$formula_content" | sed \
  -e "s|url \"https://github.com/Goldziher/spikard/archive/.*\.tar\.gz\"|url \"https://github.com/Goldziher/spikard/archive/$tag.tar.gz\"|" \
...
cd "$tap_dir"
git config user.name "spikard-bot"
git config user.email "bot@spikard.dev"

if git diff --quiet Formula/spikard.rb; then
  echo "No changes to formula"
  exit 0
fi

git add Formula/spikard.rb
git commit -m "chore(homebrew): update spikard to $version
