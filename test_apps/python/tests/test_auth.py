...
def test_api_key_authentication_invalid_key(client) -> None:
    ...
    data = response.json()
    assert data == {"detail": "The provided API key is not valid", "status": 401, "title": "Invalid API key", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_api_key_authentication_missing_header(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Expected 'X-API-Key' header or 'api_key' query parameter with valid API key", "status": 401, "title": "Missing API key", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_api_key_rotation_old_key_still_valid(client) -> None:
    ...
    data = response.json()
    assert data == {"data": "sensitive information", "message": "Access granted"}
    assert response.headers["x-api-key-deprecated"] == "true"
...
def test_bearer_token_without_prefix(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Authorization header must use Bearer scheme: 'Bearer <token>'", "status": 401, "title": "Invalid Authorization header format", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_authentication_expired_token(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Token has expired", "status": 401, "title": "JWT validation failed", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_authentication_invalid_audience(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Token audience is invalid", "status": 401, "title": "JWT validation failed", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_authentication_invalid_signature(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Token signature is invalid", "status": 401, "title": "JWT validation failed", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_authentication_missing_authorization_header(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Expected 'Authorization: Bearer <token>'", "status": 401, "title": "Missing or invalid Authorization header", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_invalid_issuer(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Token issuer is invalid, expected 'https://auth.example.com'", "status": 401, "title": "JWT validation failed", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_malformed_token_format(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "Malformed JWT token: expected 3 parts separated by dots, found 2", "status": 401, "title": "Malformed JWT token", "type": "https://spikard.dev/errors/unauthorized"}
...
def test_jwt_missing_required_custom_claims(client) -> None:
    ...
    data = response.json()
    assert data == {"detail": "Required claims 'role' and 'permissions' missing from JWT", "status": 403, "title": "Forbidden", "type": "https://spikard.dev/errors/forbidden"}
...
def test_jwt_not_before_claim_in_future(client) -> None:
    ...
    assert response.status_code == 401
    data = response.json()
    assert data == {"detail": "JWT not valid yet, not before claim is in the future", "status": 401, "title": "JWT validation failed", "type": "https://spikard.dev/errors/unauthorized"}
