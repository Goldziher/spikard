// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:e15799d9c470bee944e92a07d5b845023c70a3bd15f2070d5ee8cc1b24bc7399
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/sample_crate-dev/alef

import { describe, expect, it } from 'vitest';


describe('status_codes', () => {  it('413_payload_too_large: Request with body exceeding max size should return 413', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/19_413_payload_too_large`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', body: JSON.stringify({ data: "" }) });
    expect(response.status).toBe(413);    const data = await response.json();
    expect(data).toEqual({ error: "Payload Too Large", message: "Request body size exceeds maximum allowed size of 1024 bytes" });  });

  it('ok_success: Tests standard 200 OK response for successful GET request', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/200_ok_success`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ id: 1, name: "Item 1" });  });

  it('created_resource_created: Tests 201 Created response for successful POST request', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/201_created_resource_created`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify({ name: "New Item" }) });
    expect(response.status).toBe(201);    const data = await response.json();
    expect(data).toEqual({ id: 1, name: "New Item" });  });

  it('accepted_request_accepted_for_processing: Tests 202 Accepted for async processing', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/202_accepted_request_accepted_for_processing`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify({ task: "process_data" }) });
    expect(response.status).toBe(202);    const data = await response.json();
    expect(data).toEqual({ message: "Task accepted for processing", task_id: "abc123" });  });

  it('no_content_success_with_no_body: Tests 204 No Content response for successful DELETE', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/204_no_content_success_with_no_body`;
    const response = await fetch(mockUrl, { method: 'DELETE', redirect: 'manual' });
    expect(response.status).toBe(204);  });

  it('414_uri_too_long: Request with excessively long URI should return 414', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/20_414_uri_too_long`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({  });  });

  it('431_request_header_fields_too_large: Request with excessively large headers should return 431', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/21_431_request_header_fields_too_large`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-Large-Header": "",
    } });
    expect(response.status).toBe(431);    const data = await response.json();
    expect(data).toEqual({ error: "Request Header Fields Too Large", message: "Request headers exceed maximum allowed size of 8192 bytes" });  });

  it('503_service_unavailable: Service temporarily unavailable should return 503 with Retry-After', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/23_503_service_unavailable`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(503);    const data = await response.json();
    expect(data).toEqual({ error: "Service Unavailable", message: "The service is temporarily unavailable. Please try again later." });    expect(response.headers.get('retry-after')).toBe('0');  });

  it('not_modified_cached_content_valid: Tests 304 Not Modified for cached resources', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/304_not_modified_cached_content_valid`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "If-None-Match": "\"abc123\"",
    } });
    expect(response.status).toBe(304);  });

  it('temporary_redirect_method_preserved: Tests 307 temporary redirect with method preservation', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/307_temporary_redirect_method_preserved`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify({  }) });
    expect(response.status).toBe(307);    const data = await response.json();
    expect(data).toEqual({  });    expect(response.headers.get('location')).toBe('/target-post');  });

  it('bad_request_invalid_request: Tests 400 Bad Request for malformed request', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/400_bad_request_invalid_request`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify("not valid json") });
    expect(response.status).toBe(400);    const data = await response.json();
    expect(data).toEqual({ detail: "Invalid request format" });  });

  it('not_found_resource_not_found: Tests 404 Not Found for non-existent resource', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/404_not_found_resource_not_found`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(404);    const data = await response.json();
    expect(data).toEqual({ detail: "Item not found" });  });

  it('request_timeout: Tests 408 status code when request takes too long', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/408_request_timeout`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify({ data: "large_data" }) });
    expect(response.status).toBe(408);    const data = await response.json();
    expect(data).toEqual({ detail: "Request timeout" });    expect(response.headers.get('connection')).toBe('close');  });

  it('unprocessable_entity_validation_error: Tests 422 for validation errors (Pydantic)', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/422_unprocessable_entity_validation_error`;
    const response = await fetch(mockUrl, { method: 'POST', redirect: 'manual', headers: {
      "Content-Type": "application/json",
    }, body: JSON.stringify({ price: "not a number" }) });
    expect(response.status).toBe(422);    const data = await response.json();
    expect(data).toEqual({ detail: "1 validation error in request", errors: [{ input: { price: "not a number" }, loc: ["body", "name"], msg: "Field required", type: "missing" }], status: 422, title: "Request Validation Failed", type: "https://spikard.dev/errors/validation-error" });  });

});
