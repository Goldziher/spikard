...
describe('problem_details', () => {  it('problem_details_400_bad_request: Tests ProblemDetails for 400 Bad Request status', async () => {
  ...
  it('problem_details_with_type_uri: Tests ProblemDetails includes type URI for error classification', async () => {
    ...
    expect(response.status).toBe(422);    const data = await response.json();
    expect(data).toEqual({ detail: "1 validation error in request", errors: [{ ctx: { ge: 1 }, input: -5, loc: ["body", "value"], msg: "Input should be greater than or equal to 1", type: "greater_than_equal" }], status: 422, title: "Request Validation Failed", type: "https://spikard.dev/errors/validation-error" });  });
