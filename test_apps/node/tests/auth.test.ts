// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:5f684838fd97dd756f5222172e52338d8e7e329b935b9feee8a65355911b2753
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef

import { describe, expect, it } from 'vitest';


describe('auth', () => {  it('api_key_authentication_invalid_key: Tests API key authentication failure when provided key is not in the valid keys list', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_authentication_invalid_key`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Key": "invalid_key_12345",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "The provided API key is not valid", status: 401, title: "Invalid API key", type: "https://spikard.dev/errors/unauthorized" });  });

  it('api_key_authentication_missing_header: Tests API key authentication failure when required header is not provided', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_authentication_missing_header`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Expected 'X-API-Key' header or 'api_key' query parameter with valid API key", status: 401, title: "Missing API key", type: "https://spikard.dev/errors/unauthorized" });  });

  it('api_key_authentication_valid_key: Tests API key authentication with a valid API key in custom header', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_authentication_valid_key`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Key": "sk_test_123456",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ data: "sensitive information", message: "Access granted" });  });

  it('api_key_custom_header_x_api_token: Tests API key authentication with custom X-API-Token header', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_custom_header_x_api_token`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Token": "token_xyz789",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ resource: "data" });  });

  it('api_key_in_query_parameter: Tests API key authentication when key is provided as query parameter instead of header', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_in_query_parameter`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ data: "sensitive information", message: "Access granted" });  });

  it('api_key_multiple_valid_keys: Tests API key authentication accepts multiple valid keys', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_multiple_valid_keys`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Key": "key_staging_001",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Authenticated with staging key" });  });

  it('api_key_rotation_old_key_still_valid: Tests API key authentication during rotation period when old key remains valid alongside new key', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_rotation_old_key_still_valid`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Key": "sk_test_old_123456",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ data: "sensitive information", message: "Access granted" });    expect(response.headers.get('x-api-key-deprecated')).toBe('true');  });

  it('api_key_with_custom_header_name: Tests API key authentication with a custom header name (X-API-Token instead of X-API-Key)', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/api_key_with_custom_header_name`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "X-API-Token": "sk_test_123456",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ data: "sensitive information", message: "Access granted" });  });

  it('bearer_token_without_prefix: Tests JWT rejection when token is provided without \'Bearer \' prefix in Authorization header', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/bearer_token_without_prefix`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDZ9.8yXqZ9jKCR0BwqJc7pN_QvD3mYLxHfWzUeIaGkTnOsA",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Authorization header must use Bearer scheme: 'Bearer <token>'", status: 401, title: "Invalid Authorization header format", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_audience_claim_aud_field: Tests JWT validation of audience (aud) claim', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_audience_claim_aud_field`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDYsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSJdfQ.test",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted" });  });

  it('jwt_authentication_expired_token: Tests JWT authentication failure when token has expired (exp claim in the past)', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_authentication_expired_token`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoxNjAwMDAwMDAwLCJpYXQiOjE1OTAwMDAwMDB9.n4oBw9XuO2aAJWi1e4Bz9Y_m2iEyJHGAODcetNuwYFo",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Token has expired", status: 401, title: "JWT validation failed", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_authentication_invalid_audience: Tests JWT authentication failure when token audience claim does not match required audience', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_authentication_invalid_audience`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNTM0MDIzMDA3OTk5LCJpYXQiOjE3MzEyNTIwMDAsImF1ZCI6WyJodHRwczovL3dyb25nLXNlcnZpY2UuY29tIl19.YR2a9fSJjhen7ksYFI2djSBSC7Pc29FDCloBGhkj3kU",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Token audience is invalid", status: 401, title: "JWT validation failed", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_authentication_invalid_signature: Tests JWT authentication failure when token signature does not match the secret', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_authentication_invalid_signature`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNTM0MDIzMDA3OTksImlhdCI6MTczMTI1MjAwMH0.invalid_signature_here",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Token signature is invalid", status: 401, title: "JWT validation failed", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_authentication_missing_authorization_header: Tests JWT authentication failure when Authorization header is not provided', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_authentication_missing_authorization_header`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual' });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Expected 'Authorization: Bearer <token>'", status: 401, title: "Missing or invalid Authorization header", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_authentication_valid_token: Tests JWT authentication with a valid token containing correct signature and claims', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_authentication_valid_token`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDYsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSJdLCJpc3MiOiJodHRwczovL2F1dGguZXhhbXBsZS5jb20ifQ.TpRpCJeXROQ12-ehRCVZm6EgN7Dn6QpfoekxJvnzgQg",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted", user_id: "user123" });  });

  it('jwt_config_algorithm_rs256: Tests JWT validation with RS256 asymmetric algorithm', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_config_algorithm_rs256`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDZ9.test_signature",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted" });  });

  it('jwt_config_leeway_seconds: Tests JWT validation with leeway for clock skew', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_config_leeway_seconds`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoxNzYyNzgzOTQ2LCJpYXQiOjE2MDAwMDAwMDB9.test",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted with leeway" });  });

  it('jwt_expiration_claim_exp_field: Tests JWT validation of expiration (exp) claim', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_expiration_claim_exp_field`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDZ9.test",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted" });  });

  it('jwt_invalid_issuer: Tests JWT rejection when issuer claim doesn\'t match expected value', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_invalid_issuer`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE2MDAwMDAwMDAsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSJdLCJpc3MiOiJodHRwczovL2V2aWwuY29tIn0.mbL5L04_hpaaiz0SPABap6ZWfBLu18aiexBjzwQ1nnA",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Token issuer is invalid, expected 'https://auth.example.com'", status: 401, title: "JWT validation failed", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_issuer_claim_iss_field: Tests JWT validation of issuer (iss) claim', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_issuer_claim_iss_field`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDYsImlzcyI6Imh0dHBzOi8vYXV0aC5leGFtcGxlLmNvbSJ9.test",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted" });  });

  it('jwt_malformed_token_format: Tests JWT rejection when token doesn\'t have the required 3-part structure (header.payload.signature)', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_malformed_token_format`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer invalid.token",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "Malformed JWT token: expected 3 parts separated by dots, found 2", status: 401, title: "Malformed JWT token", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_missing_required_custom_claims: Tests JWT rejection when required custom claims (role, permissions) are missing', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_missing_required_custom_claims`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDYsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSJdLCJpc3MiOiJodHRwczovL2F1dGguZXhhbXBsZS5jb20ifQ.TpRpCJeXROQ12-ehRCVZm6EgN7Dn6QpfoekxJvnzgQg",
    } });
    expect(response.status).toBe(403);    const data = await response.json();
    expect(data).toEqual({ detail: "Required claims 'role' and 'permissions' missing from JWT", status: 403, title: "Forbidden", type: "https://spikard.dev/errors/forbidden" });  });

  it('jwt_not_before_claim_in_future: Tests JWT rejection when nbf (not before) claim is in the future', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_not_before_claim_in_future`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE2MDAwMDAwMDAsIm5iZiI6MjYyNjc4Mzk0NiwiYXVkIjpbImh0dHBzOi8vYXBpLmV4YW1wbGUuY29tIl0sImlzcyI6Imh0dHBzOi8vYXV0aC5leGFtcGxlLmNvbSJ9.hG4I76_3kJfsbJ_jmxoP1NSYnkcqdyBFcPpdo-jYU4E",
    } });
    expect(response.status).toBe(401);    const data = await response.json();
    expect(data).toEqual({ detail: "JWT not valid yet, not before claim is in the future", status: 401, title: "JWT validation failed", type: "https://spikard.dev/errors/unauthorized" });  });

  it('jwt_subject_claim_sub_field: Tests JWT validation with subject (sub) claim', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_subject_claim_sub_field`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyLWFiYzEyMyIsImV4cCI6MjYyNjc4Mzk0NiwiaWF0IjoxNzYyNzgzOTQ2fQ.xYz9QaJkL5M2NpQ8RsT1UvW3XyZ4AaBbCcDdEeFfGgHhIiJ",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ user_id: "user-abc123" });  });

  it('jwt_with_multiple_audiences: Tests JWT validation when token has multiple audiences and one must match', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/jwt_with_multiple_audiences`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE2MDAwMDAwMDAsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSIsImh0dHBzOi8vYWRtaW4uZXhhbXBsZS5jb20iXSwiaXNzIjoiaHR0cHM6Ly9hdXRoLmV4YW1wbGUuY29tIn0.9MBL_XccGXfu9cDUnCpQruDMOl2hHYydzeGn-20dQOs",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ message: "Access granted", user_id: "user123" });  });

  it('multiple_authentication_schemes_jwt_precedence: Tests authentication when both JWT and API key are provided, JWT takes precedence', async () => {
    const mockUrl = `${process.env.MOCK_SERVER_URL}/fixtures/multiple_authentication_schemes_jwt_precedence`;
    const response = await fetch(mockUrl, { method: 'GET', redirect: 'manual', headers: {
      "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwiZXhwIjoyNjI2NzgzOTQ2LCJpYXQiOjE3NjI3ODM5NDYsImF1ZCI6WyJodHRwczovL2FwaS5leGFtcGxlLmNvbSJdLCJpc3MiOiJodHRwczovL2F1dGguZXhhbXBsZS5jb20ifQ.TpRpCJeXROQ12-ehRCVZm6EgN7Dn6QpfoekxJvnzgQg",
      "X-API-Key": "sk_test_123456",
    } });
    expect(response.status).toBe(200);    const data = await response.json();
    expect(data).toEqual({ auth_method: "jwt", message: "Access granted", user_id: "user123" });  });

});
