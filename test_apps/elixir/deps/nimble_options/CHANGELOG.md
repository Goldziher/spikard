...
- Make the `NimbleOptions.t/0` type _public_ (instead of opaque). This helps with Dialyzer issues when ysing `NimbleOptions.new!/1` at compile time.

## v1.0.0 (2023-03-08)

- Add support for a `{:struct, struct_name}` type specifier
- Add support for the `:type_doc` option
- Turn `t:NimbleOptions.t/0` into an _opaque_ type
