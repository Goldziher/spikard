...
defmodule E2e.ProblemDetailsTest do
  ...
  describe "problem_details_400_bad_request" do
    test "Tests ProblemDetails for 400 Bad Request status" do
      {:ok, response} = Req.post(url: (System.get_env("SUT_URL") || mock_server_url()) <> "/fixtures/problem_details_400_bad_request", connect_options: [protocols: [:http1]], json: %{"data" => "example"}, headers: [{"Content-Type", "application/json"}])
